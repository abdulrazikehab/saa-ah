...
export const getProfessionalErrorMessage = (
  ...
  const isGenericMessage = !rawMessage || 
    rawMessage.toLowerCase().includes('error') || 
    rawMessage.toLowerCase().includes('request') || 
    /^unauthorized$/i.test(rawMessage);

  if (status && status >= 400) {
    // Special handling for 401 during Login - Show "Invalid credentials" instead of "Session expired"
    if (status === 401 && context?.operation && (context.operation === 'login' || context.operation === 'تسجيل الدخول')) {
       return {
          title: isRTL ? 'خطأ في الدخول' : 'Login Failed',
    ...
    // For 400 errors, check if we have validation error details
    if (status === 400 && error && typeof error === 'object') {
      const err = error as Record<string, unknown>;
      // Check if error has validation details (NestJS ValidationPipe format)
      if (err.message && typeof err.message === 'object') {
        ...
        if (Array.isArray(msgObj.message)) {
          const validationErrors = msgObj.message as string[];
          // Join validation errors into a readable message
          const validationMsg = validationErrors.join('. ');
          return {
            title: statusMsg.title,
            description: validationMsg || statusMsg.description
          };
        }
        // Check if it's a single validation message
      ...
      // Check if message is directly an array (alternative format)
      if (Array.isArray(err.message)) {
        const validationErrors = err.message as string[];
        const validationMsg = validationErrors.join('. ');
        return {
          title: statusMsg.title,
          description: validationMsg || statusMsg.description
    ...
    // unless it's a generic "Bad Request"
    // DEBUG: Allow 500 errors to show through if they look like debug messages
    if (rawMessage && !isGenericMessage && (status !== 500 || rawMessage.includes('| Stack:'))) {
      return {
        title: statusMsg.title,
  ...
  const commonMaps: Record<string, { ar: string; en: string }> = {
    ...
    'employee with this email already exists': { ar: 'يوجد موظف مسجل بهذا البريد الإلكتروني بالفعل.', en: 'An employee with this email already exists.' },
    'email already exists': { ar: 'البريد الإلكتروني مسجل مسبقاً.', en: 'This email is already registered.' },
    'store is not active': { ar: 'المتجر غير نشط حالياً. يرجى التواصل مع الدعم.', en: 'The store is currently inactive. Please contact support.' },
    'customer not found': { ar: 'حساب العميل غير موجود.', en: 'Customer account not found.' },
    'invalid credentials': { ar: 'بيانات الدخول غير صحيحة.', en: 'Invalid email or password.' },
    'user no longer exists': { ar: 'لم يعد هذا المستخدم موجوداً.', en: 'This user account no longer exists.' },
    // 'unauthorized' removed - duplicate
    'amount must be greater': { ar: 'يجب أن يكون المبلغ أكبر من صفر.', en: 'The amount must be greater than zero.' },
    'insufficient balance': { ar: 'رصيد المحفظة غير كافي.', en: 'Insufficient wallet balance.' },
  };
