...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refreshCart = React.useCallback(async () => {
    ...
    try {
      ...
      // Handle 404 (Cart not found) gracefully - treat as empty cart
      const errorMessage = getErrorMessage(error);
      const isNotFound = errorMessage.includes('not found') || 
                         errorMessage.includes('404') || 
                         (error && typeof error === 'object' && 'status' in error && (error as any).status === 404) ||
                         errorMessage.includes('العنصر الذي تبحث عنه غير متوفر'); // Backend Arabic 404 message

      if (isNotFound) {
        console.log('🛒 CartContext: Cart not found (404), treating as empty');
        setCart(null);
