...
export function ThemeCustomizer() {
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem('custom-theme-config');
    if (savedTheme) {
      try {
      ...
      // Ensure default theme is applied if no saved theme exists
      applyTheme(DEFAULT_THEME);
    }
  }, []);
  ...
  const handleReset = () => {
    ...
    applyTheme(DEFAULT_THEME);
    localStorage.removeItem('custom-theme-config');
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button 
          variant="outline" 
          size="icon" 
          className="fixed bottom-4 right-4 z-50 h-12 w-12 rounded-full shadow-lg border-primary/20 bg-background/80 backdrop-blur-sm hover:bg-primary/10 hover:border-primary transition-all duration-300"
        >
