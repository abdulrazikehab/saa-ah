...
export const DashboardSidebar = ({ className, collapsed = false, onToggleCollapse }: DashboardSidebarProps) => {
  ...
  // Use static keys for section tracking (not translated strings)
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    'home': true,
    'salesAndOrders': true,
    'reports': true,
    'storeSettings': true,
    'contentAndDesign': true,
    'advanced': false,
    'marketing': true,
  });
  ...
  // Simulating fetching the active version from backend/config
  const [activeVersionConfig, setActiveVersionConfig] = useState<string[] | null>(null);

  useEffect(() => {
    // In a real app, this would come from `config.activeVersion` or similar
    // For this demo, we'll try to find a default version in localStorage
    try {
        const storedVersions = localStorage.getItem('system_versions');
        const userPref = localStorage.getItem('tenant_dashboard_version_preference');

        if (storedVersions) {
            const parsed = JSON.parse(storedVersions) as Array<{ id: string; isDefault: boolean; visibleTenantTabs: string[]; name: string }>;
            
            // Find version: User Preference -> Default -> First
            let activeVer = null;
            if (userPref) {
                activeVer = parsed.find(v => v.id === userPref);
            }
            if (!activeVer) {
                activeVer = parsed.find(v => v.isDefault) || parsed[0];
            }

            if (activeVer && activeVer.visibleTenantTabs) {
                console.log('Applied Dashboard Version:', activeVer.name);
                setActiveVersionConfig(activeVer.visibleTenantTabs);
            }
        }
    } catch (e) {
  ...
  // Filter navigation items based on permissions AND version config
  // Wrap in useCallback to avoid useMemo dependency issues
  const filterNavigationItems = useCallback((items: Array<{ name: string; href: string; icon: LucideIcon; badge?: string | null }>) => {
    return items.filter(item => {
      // 1. Permission Check
      // Always allow dashboard home if not explicitly restricted (though usually home is always allowed)
      const allowedByPermission = (
        item.href === '/dashboard' || 
        item.href === '/dashboard/pages' || 
        item.href === '/dashboard/app-builder' || 
        item.href === '/dashboard/apps'
      ) || canAccessRoute(item.href);
      
      if (!allowedByPermission) return false;

      // 2. Version/Plan Check (Configurability)
      if (activeVersionConfig) {
        // Derive ID from href to match VersionsTypes.ts IDs
        let id = item.href.split('/').pop() || '';
        if (item.href === '/dashboard') id = 'dashboard';
        
        // Always allow specific important routes even if not in version config
        const isEssentialRoute = item.href === '/dashboard/app-builder' || item.href === '/dashboard/apps' || item.href === '/dashboard/themes';
        
        // If the ID is NOT in the allowed list, hide it (unless it's an essential route)
        if (!isEssentialRoute && !activeVersionConfig.includes(id)) {
            return false;
        }
      }
  ...
  }, [canAccessRoute, activeVersionConfig]);

  // Define sections with static keys for state tracking and translated display titles
  // Use useMemo to ensure sections are recreated when language changes
  const navigationSections: (NavigationSection & { key: string })[] = useMemo(() => [
    {
      key: 'home',
    ...
    },
    {
      ...
      items: filterNavigationItems([
        ...
        { name: t('dashboard.sidebar.inventory'), href: '/dashboard/inventory', icon: Package, badge: null },
        { name: t('dashboard.wallet.pageTitle', 'Banks and Transactions'), href: '/dashboard/banks', icon: Receipt, badge: null },
        { name: t('supplierProducts.title'), href: '/dashboard/supplier-products', icon: Building2, badge: null },
    ...
    },
    {
      ...
      items: filterNavigationItems([
        { name: t('dashboard.sidebar.generalReports', 'التقارير العامة'), href: '/dashboard/reports', icon: BarChart3, badge: null },
        { name: t('dashboard.sidebar.customerReports', 'تقارير العملاء'), href: '/dashboard/customer-reports', icon: Users, badge: null },
        { name: t('dashboard.sidebar.customerBalances', 'أرصدة العملاء'), href: '/dashboard/customer-balances', icon: Wallet, badge: null },
        { name: t('dashboard.sidebar.productReports', 'تقارير المنتجات'), href: '/dashboard/product-reports', icon: BarChart3, badge: null },
        { name: t('dashboard.sidebar.paymentReports', 'تقارير المدفوعات'), href: '/dashboard/payment-reports', icon: CreditCard, badge: null },
      ]),
    },
        {
      key: 'storeSettings',
      ...
      items: filterNavigationItems([
        ...
        { name: t('dashboard.sidebar.storeManagement'), href: '/dashboard/management', icon: Building2, badge: null },
        { name: t('dashboard.sidebar.suppliers'), href: '/dashboard/settings/suppliers', icon: Truck, badge: null },
     ...
     {
      key: 'contentAndDesign',
      title: t('dashboard.sidebar.contentAndDesign'),
      items: filterNavigationItems([
        { name: t('dashboard.sidebar.pages'), href: '/dashboard/pages', icon: FileText, badge: null },
        { name: t('dashboard.sidebar.themes', 'المظهر'), href: '/dashboard/themes', icon: Palette, badge: null },
        { name: t('dashboard.sidebar.myApp', 'تطبيقي'), href: '/dashboard/app-builder', icon: Smartphone, badge: null },
          ...
          {!collapsed && (
            <div className="flex flex-col">
              <span className="text-lg font-bold gradient-text truncate max-w-[140px]">
                {config?.storeNameAr || config?.storeName || config?.tenantName || "Koun"}
              </span>
      ...
      <ScrollArea className="flex-1 px-4 py-4">
        <nav className="space-y-2">
          {navigationSections.map((section) => {
            ...
            const isExpanded = collapsed ? false : (expandedSections[section.key] ?? true);
