...
import { Loader2, Plus, Search, FileText, Heart, ShoppingCart, X, Users, Upload, Calendar, TrendingUp, BarChart3, User, CreditCard, DollarSign, Wallet, Info, AlertCircle, MessageSquare, RefreshCw } from 'lucide-react';
...
export function EmployeesPageSection({ props }: { props: Record<string, unknown> }) {
  ...
  const handleAddEmployee = async (e?: React.FormEvent) => {
    ...
    if (!newEmployee.name || !newEmployee.username || (!editingEmployee && !newEmployee.password) || !newEmployee.phone) {
      toast({
    ...
    try {
      ...
      if (editingEmployee) {
        ...
        // Update permissions separately if needed, or backend handles it
        // The backend endpoint for update might be different or same with PUT
        // Currently backend only has POST /merchant/employees and DELETE /merchant/employees/:id
        // We need to check if there is a PUT endpoint.
        // Looking at MerchantEmployeesController (Core), there is NO PUT endpoint for update!
        // But CustomerEmployeesController (Auth) has PUT :id/permissions
        
        // Wait, I need to implement PUT in MerchantEmployeesController first?
        // Or maybe I should just update permissions?
        
        // Let's assume for now we want to update basic info + permissions.
        // Since backend doesn't support full update yet, I might need to add it.
        // But for now, let's try to use what we have or add the endpoint.
        
        // Actually, looking at the previous file view of MerchantEmployeesController, 
        // it has @Delete but NO @Put for updating employee details!
        // It only has @Post for create.
        
        // So I need to ADD the update endpoint to MerchantEmployeesController first.
        // But I am in frontend file now.
        
        // I will implement the frontend logic assuming the endpoint exists (or I will add it next).
        ...
        // Create new employee
        await apiClient.fetch(`${apiClient.coreUrl}/merchant/employees`, {
          method: 'POST',
          body: JSON.stringify(employeeData),
          requireAuth: true
        });
      ...
      toast({
        title: 'خطأ',
        description: (error as { message?: string })?.message || 'فشل حفظ بيانات الموظف. يرجى المحاولة مرة أخرى.',
        variant: 'destructive',
      });
    } finally {
      setSubmitting(false);
            ...
            <form onSubmit={handleAddEmployee} className="space-y-4 py-4">
              ...
              {customers.length > 0 && (
                <div className="col-span-2">
                  <Label htmlFor="customerSelect">{t('sections.employeesPage.form.selectCustomer', 'Select Customer (Optional)')}</Label>
                  <select
                    id="customerSelect"
                    onChange={(e) => {
                  ...
                  >
                    <option value="">{t('sections.employeesPage.form.selectCustomerPlaceholder', '-- Select Customer --')}</option>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {t('sections.employeesPage.form.selectCustomerHint', 'Name and phone will be auto-filled')}
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="empName">{t('sections.employeesPage.form.name', 'Name')} *</Label>
                  <Input
                    id="empName"
                    value={newEmployee.name}
                    onChange={(e) => setNewEmployee({ ...newEmployee, name: e.target.value })}
                    placeholder={t('sections.employeesPage.form.namePlaceholder', 'Employee Name')}
                  />
                </div>
                <div>
                  <Label htmlFor="empUsername">{t('sections.employeesPage.form.username', 'Username')} *</Label>
                  <Input
                    id="empUsername"
                    value={newEmployee.username}
                    onChange={(e) => setNewEmployee({ ...newEmployee, username: e.target.value })}
                    placeholder={t('sections.employeesPage.form.usernamePlaceholder', 'Username')}
                  />
                </div>
                <div>
                  <Label htmlFor="empPassword">
                    {t('sections.employeesPage.form.password', 'Password')} 
                    {editingEmployee ? ` (${t('common.optional', 'Optional')})` : ' *'}
                  </Label>
                  <Input
                    id="empPassword"
                    type="password"
                    ...
                    placeholder={t('sections.employeesPage.form.passwordPlaceholder', 'Password')}
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    placeholder={t('sections.employeesPage.form.phonePlaceholder', 'Phone Number')}
                  />
                </div>
              </div>
