...
export function PagesSidebar({ isOpen, onClose, logoUrl, storeName }: PagesSidebarProps) {
  ...
  const getPageIcon = (page: NavigationPage) => {
    ...
    if (title.includes('employee') || title.includes('staff') || title.includes('موظف')) return Users;
    if (title.includes('wallet') || title.includes('balance') || title.includes('رصيد') || title.includes('محفظة')) return Wallet;
    if (title.includes('report') || title.includes('تقارير')) return BarChart3;
    if (title.includes('profile') || title.includes('ملف')) return User;
    return FileText;
  };
