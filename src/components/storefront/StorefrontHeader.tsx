...
import { 
  ...
  FileText, PanelLeftOpen, Wallet, ShoppingBag, CheckCircle, AlertCircle, Info, ShieldCheck
} from 'lucide-react';
...
export function StorefrontHeader({ cartItemCount: propCount = 0, onSearch }: StorefrontHeaderProps) {
  ...
  useEffect(() => {
    // Don't load wallet balance on login/auth pages to prevent redirect loops
    ...
    loadWalletBalance();
  }, [customerData, location.pathname]);

  // Load store notifications (real events only)
  useEffect(() => {
    // Don't load notifications on login/auth pages to prevent redirect loops
    const currentPath = location.pathname;
    const isAuthPage = currentPath === '/login' || 
                       currentPath === '/auth/login' || 
                       currentPath === '/signup' || 
    ...
    const loadCurrencies = async () => {
      try {
        ...
        if (Array.isArray(response)) {
          setCurrencies(response.map((c: { code: string; exchangeRate: string | number }) => ({
            code: c.code,
  ...
  const convertCurrency = (amount: number, fromCurrency: string, toCurrency: string) => {
    ...
    // Assuming rates are relative to base currency (1 BaseUnit = Rate TargetUnit)
    // ValueInBase = Amount / FromRate
    // ValueInTarget = ValueInBase * ToRate
    // So: NewAmount = Amount * (ToRate / FromRate)
    
    // WAIT! In CurrencySettings.tsx, exchangeRate is usually "1 BaseUnit = X TargetUnit".
    // If Base=USD. SAR=3.75.
    // 1 USD = 3.75 SAR.
    // If I have 100 SAR (From=SAR, Rate=3.75) -> convert to USD (To=USD, Rate=1.0).
    // AmountInBase = AmountInFrom / FromRate = 100 / 3.75 = 26.66 USD.
    // AmountInTarget = AmountInBase * ToRate = 26.66 * 1.0 = 26.66 USD.
  ...
  const handleLogout = (e?: React.MouseEvent) => {
    ...
    try {
      ...
      // Force reload to login even if there's an error
      const search = window.location.search;
      window.location.href = `/login${search}`;
    }
  };
  ...
  const toggleLanguage = () => {
    ...
    // Use centralized setLanguage function for consistency
      ...
      <header className={cn(
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between gap-4">
            {/* Pages Sidebar Toggle Button - Always visible */}
            <button
              ...
              aria-label={language === 'ar' ? 'فتح قائمة الصفحات' : 'Open pages menu'}
            >
                          ...
                          categories.map((category: Category, index: number) => (
                            ...
                            >
                              {category.image ? (
                                <img 
                                  src={category.image} 
                                  alt={language === 'ar' ? (category.nameAr || category.name) : category.name}
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              {/* Pages Sidebar Button (Mobile only) - Always visible */}
              <button
                onClick={() => setShowPagesSidebar(true)}
                className={cn(
              ...
              {customerData && siteConfig && siteConfig.settings && siteConfig.settings.storeType === 'DIGITAL_CARDS' && (
                ...
                >
                  ...
                  <span>{formatCurrency(convertCurrency(walletBalance, walletCurrency, settings?.currency || siteConfig?.settings?.currency || 'SAR'), settings?.currency || siteConfig?.settings?.currency || 'SAR')}</span>
                </Link>
              )}
                      ...
                      >
                        <div className={cn(
                          ...
                          <div className="p-3 border-b border-border/50 gradient-mesh">
                            ...
                            {customerData?.isEmployee && customerData?.employerEmail && (
                              <p className="text-xs text-primary truncate mt-1">
                                {language === 'ar' ? 'موظف لدى: ' : 'Employee of: '}{customerData.employerEmail}
                              </p>
                            )}
                          </div>
                          <div className="py-1">
                            ...
                            >
                              ...
                              <span>{language === 'ar' ? 'مخزون بطاقاتي' : 'My Inventory'}</span>
                            </Link>
                            <Link
                              to="/account/profile"
                              onClick={() => setShowUserMenu(false)}
        ...
        {showMobileMenu && (
          <div className="lg:hidden border-t border-border/30 glass-effect-strong animate-slide-down">
            <nav className="container mx-auto px-4 py-4 flex flex-col gap-2">
              ...
              {/* Pages Button in Mobile - Always visible */}
              <button
                onClick={() => {
                  setShowMobileMenu(false);
                  setShowPagesSidebar(true);
                }}
                className="flex items-center gap-2 px-4 py-2 rounded-xl font-medium transition-all duration-300 hover:bg-muted/50 text-left w-full"
              >
              ...
              {customerData && siteConfig && siteConfig.settings && siteConfig.settings.storeType === 'DIGITAL_CARDS' && (
                ...
                >
                  <Wallet className="h-4 w-4" />
                  <span>{language === 'ar' ? 'الرصيد' : 'Balance'}: {formatCurrency(walletBalance, settings?.currency || siteConfig?.settings?.currency || 'SAR')}</span>
                </Link>
              )}
