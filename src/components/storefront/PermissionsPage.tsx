...
export default function PermissionsPage({ props }: { props?: Record<string, unknown> }) {
  const { user } = useAuth();
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            {isCustomer 
              ? (props?.titleAr as string) || (props?.title as string) || 'صلاحيات العملاء' 
              : (props?.titleEn as string) || (props?.title as string) || 'My Permissions'}
          </h1>
          <p className="text-lg text-gray-600">
            {isCustomer 
              ? (props?.subtitleAr as string) || (props?.subtitle as string) || 'عرض ما يمكنك رؤيته والوصول إليه في المتجر'
              : (props?.subtitleEn as string) || (props?.subtitle as string) || 'View what you can see and access in the store dashboard'
            }
          </p>
        </div>
