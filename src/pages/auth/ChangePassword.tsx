...
export default function ChangePassword() {
  ...
  useEffect(() => {
    ...
    const userProfile = user as unknown as { mustChangePassword?: boolean };
    const mustChange = userData.mustChangePassword || userProfile?.mustChangePassword;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check for complexity based on backend validator
      ...
      if (!hasUppercase || !hasLowercase || !hasNumber || !hasSpecial) {
        toast({
          ...
          description: 'يجب أن تحتوي كلمة المرور على حرف كبير، حرف صغير، رقم، ورمز خاص واحد على الأقل',
        });
        setLoading(false);
        return;
      }
      ...
      const err = error as { message?: string; data?: { message?: string | string[] } };
      console.error('Failed to change password:', err);
      
      let displayMessage = err?.message || 'حدث خطأ أثناء تغيير كلمة المرور. يرجى المحاولة مرة أخرى.';
      
      // Try to get specific backend error message which is often hiding in invalid 400 Generic responses
      if (err?.data?.message) {
        if (typeof err.data.message === 'string') {
          displayMessage = err.data.message;
        } else if (Array.isArray(err.data.message)) {
          displayMessage = err.data.message.join('. ');
        }
      }
