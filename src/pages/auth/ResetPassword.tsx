  ...
  useEffect(() => {
    const verifyToken = async () => {
      if (tokenParam) {
        ...
        // SECURITY FIX: Remove token from URL after capture to prevent leakage
        // via Referer headers, browser history, or logs.
        try {
          const url = new URL(window.location.href);
          url.searchParams.delete('token');
          window.history.replaceState({}, '', url.toString());
        } catch (e) {
          console.error('Failed to clear token from URL:', e);
        }
        try {
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10">
          <Link to="/" className="flex items-center gap-4 group">
            <div className="w-16 h-16 rounded-xl overflow-hidden bg-white/10 backdrop-blur-sm border border-white/20 shadow-lg flex items-center justify-center bg-white">
              <img 
                src={logoUrl} 
                alt={brandName} 
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-gray-50 dark:bg-gray-900">
        <div className="w-full max-w-md">
          <Link to="/" className="flex lg:hidden items-center justify-center gap-3 mb-8 group">
            <div className="w-14 h-14 rounded-xl overflow-hidden bg-white border border-border shadow-lg flex items-center justify-center">
              <img 
                src={logoUrl} 
                alt={brandName}
                className="w-full h-full object-contain p-2 group-hover:scale-110 transition-transform" 
