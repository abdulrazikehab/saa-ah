...
export default function PlansManager({ adminApiKey }: { adminApiKey?: string }) {
  ...
  const loadPlans = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await coreApi.get('/admin/master/plans', { 
        requireAuth: true, 
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await coreApi.post('/admin/master/plans', formData, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await coreApi.put(`/admin/master/plans/${editingPlan.id}`, formData, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleToggle = async (id: string) => {
    try {
      await coreApi.patch(`/admin/master/plans/${id}/toggle`, {}, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await coreApi.delete(`/admin/master/plans/${id}`, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
