...
export default function TenantManagement({ adminApiKey }: { adminApiKey?: string }) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await coreApi.post('/admin/master/tenants', formData, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleActivate = async (id: string) => {
    try {
      await coreApi.post(`/admin/master/tenants/${id}/activate`, {}, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await coreApi.delete(`/admin/master/tenants/${id}`, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const filteredTenants = tenants.filter(tenant =>
    ...
    (tenant.nameAr && tenant.nameAr.includes(searchTerm)) ||
                  ...
                  <td className="p-4">
                    <div>
                      <div className="font-medium text-white">
                        {isRTL ? (tenant.nameAr || tenant.name) : tenant.name}
                      </div>
                      ...
                      <button
                        onClick={() => {
                          ...
                          setFormData({
                            ...
                            descriptionAr: tenant.descriptionAr || '',
                            ownerEmail: '',
                            ownerPassword: ''
                          });
                        }}
            ...
            <form onSubmit={editingTenant ? handleUpdate : handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white focus:outline-none focus:border-purple-500 min-h-[80px]"
                />
              </div>
