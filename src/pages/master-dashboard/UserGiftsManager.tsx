...
export default function UserGiftsManager({ adminApiKey }: { adminApiKey?: string }) {
  ...
  const loadGifts = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await coreApi.get('/admin/master/gifts', { requireAuth: true, adminApiKey: adminApiKey });
  ...
  const loadUsers = useCallback(async () => {
    if (!adminApiKey) return;
    try {
      const response = await coreApi.get('/admin/master/users?limit=100', { requireAuth: true, adminApiKey: adminApiKey });
  ...
  const loadTenants = useCallback(async () => {
    if (!adminApiKey) return;
    try {
      const response = await coreApi.get('/admin/master/tenants', { requireAuth: true, adminApiKey: adminApiKey });
  ...
  useEffect(() => {
    ...
    loadUsers();
    loadTenants();
  }, [loadGifts, loadUsers, loadTenants]);
  ...
  const handleGrant = async (e: React.FormEvent) => {
    ...
    if (!adminApiKey) {
      toast({
        variant: 'destructive',
        title: '❌ Error',
        description: 'Admin API key is missing. Cannot grant gift.'
      });
      return;
    }
    ...
    try {
      await coreApi.post('/admin/master/gifts', giftData, { 
        requireAuth: true, 
  ...
  const handleRevoke = async (giftId: string) => {
    ...
    try {
      await coreApi.post(`/admin/master/gifts/${giftId}/revoke`, {}, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
