...
const DEFAULT_FEATURES: Omit<FeatureConfig, 'id' | 'createdAt'>[] = [
  ...
  { feature: 'welcome_tour_banner', displayName: 'Welcome Tour Banner', displayNameAr: 'بانر جولة الترحيب', category: 'core', requiredPlan: 'FREE', isGlobal: true, isEnabled: true, description: 'Show welcome tour banner on dashboard' },
  { feature: 'dashboard_promotion_banner', displayName: 'Dashboard Promotion Banner', displayNameAr: 'بانر العروض في لوحة التحكم', category: 'marketing', requiredPlan: 'FREE', isGlobal: true, isEnabled: true, description: 'Show promotional banners on dashboard' },
];

export default function FeatureControlManager({ adminApiKey }: { adminApiKey?: string }) {
  ...
  const loadFeatures = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await coreApi.get('/admin/master/features', {
        requireAuth: true,
  ...
  const loadTenantFeatures = useCallback(async () => {
    if (!adminApiKey) return;
    try {
      const response = await coreApi.get('/admin/master/feature-overrides', {
        requireAuth: true,
        adminApiKey: adminApiKey
      });
  ...
  useEffect(() => {
    loadFeatures();
    loadTenantFeatures();
  }, [loadFeatures, loadTenantFeatures]);

  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await coreApi.post('/admin/master/features', formData, { requireAuth: true, adminApiKey: adminApiKey });
      toast({
        title: '✅ Success',
      ...
      toast({
        variant: 'destructive',
        title: '❌ Error',
        description: err.response?.data?.message || 'Failed to create feature'
      });
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      if (!adminApiKey) return;
      await coreApi.put(`/admin/master/features/${editingFeature.id}`, formData, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
      ...
      toast({
        variant: 'destructive',
        title: '❌ Error',
        description: err.response?.data?.message || 'Failed to update feature'
      });
    }
  };
  ...
  const handleToggleFeature = async (id: string) => {
    try {
      if (!adminApiKey) return;
      await coreApi.post(`/admin/master/features/${id}/toggle`, {}, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
      ...
      toast({
        variant: 'destructive',
        title: '❌ Error',
        description: err.response?.data?.message || 'Failed to toggle feature'
      });
    }
  };
  ...
  const handleChangePlan = async (id: string, plan: string) => {
    try {
      if (!adminApiKey) return;
      await coreApi.put(`/admin/master/features/${id}`, { requiredPlan: plan }, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleCreateOverride = async (e: React.FormEvent) => {
    ...
    try {
      if (!adminApiKey) return;
      await coreApi.post('/admin/master/feature-overrides', overrideFormData, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
      ...
      toast({
        variant: 'destructive',
        title: '❌ Error',
        description: err.response?.data?.message || 'Failed to create override'
      });
    }
  };
  ...
  const handleDeleteOverride = async (id: string) => {
    ...
    try {
      if (!adminApiKey) return;
      await coreApi.delete(`/admin/master/feature-overrides/${id}`, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
                ...
                <div className="grid grid-cols-4 gap-2">
                  {PLAN_LEVELS.map(plan => (
                    <button
                      key={plan.value}
                      type="button"
                      onClick={() => setFormData({ ...formData, requiredPlan: plan.value as FeatureConfig['requiredPlan'] })}
