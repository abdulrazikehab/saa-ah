...
export default function ApiKeyManager({ adminApiKey }: { adminApiKey?: string }) {
  ...
  const loadApiKeys = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await coreApi.get('/admin/master/api-keys', { 
        requireAuth: true, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingApiKey) {
        ...
        await coreApi.put(`/admin/master/api-keys/${encodedId}`, { name: formData.name }, { 
          requireAuth: true, 
          adminApiKey: adminApiKey 
        });
        toast({
        ...
        const response = await coreApi.post('/admin/master/api-keys', { name: formData.name.trim() }, { 
          requireAuth: true, 
          adminApiKey: adminApiKey 
        });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      await coreApi.delete(`/admin/master/api-keys/${encodedId}`, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      const response = await coreApi.post(`/admin/master/api-keys/${encodedId}/regenerate`, {}, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      setNewApiKey(response.apiKey);
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  id="name"
                  placeholder="e.g., Koun Website, Edara Mobile App, Mobile Client"
                  value={formData.name}
