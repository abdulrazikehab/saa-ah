...
export default function PaymentGatewayManager({ adminApiKey }: { adminApiKey?: string }) {
  ...
  const loadGateways = useCallback(async () => {
    ...
    try {
      ...
      const response = await coreApi.get('/admin/master/payment-gateways', { 
        requireAuth: true, 
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await coreApi.post('/admin/master/payment-gateways', formData, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await coreApi.delete(`/admin/master/payment-gateways/${id}`, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleToggle = async (id: string) => {
    try {
      await coreApi.post(`/admin/master/payment-gateways/${id}/toggle`, {}, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await coreApi.put(`/admin/master/payment-gateways/${editingGateway.id}`, formData, { 
        requireAuth: true, 
        adminApiKey: adminApiKey 
      });
      toast({
