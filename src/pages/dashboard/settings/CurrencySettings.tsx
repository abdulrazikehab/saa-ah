...
import { Plus, Edit, Trash2, Save, X, DollarSign, RefreshCw, Check } from 'lucide-react';
...
import { currencyIconsConfig, getCurrencyIconConfig, CURRENCY_ICON_CONFIG } from '@/lib/currency-icon-config';
...
export default function CurrencySettings() {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      toast({
        title: 'نجح',
        description: 'تم حفظ إعدادات العملة بنجاح. تم تحديث أسعار الصرف تلقائياً.',
      });
      setIsSettingsDialogOpen(false);
      // Reload both settings and currencies to show updated exchange rates
  ...
  const handleEdit = (currency: Currency) => {
    ...
    const config = getCurrencyIconConfig(currency.code.toUpperCase());
    const iconValue = currency.icon || 
      (config?.type === 'svg' ? config.value : config?.type === 'unicode' ? config.value : currency.code);
    setFormData({
      code: currency.code,
      name: currency.name,
  ...
  // Auto-select icon when currency code changes
  useEffect(() => {
    if (formData.code && !formData.icon) {
      const config = getCurrencyIconConfig(formData.code.toUpperCase());
      if (config) {
        // Set icon based on type
        const iconValue = config.type === 'svg' 
          ? config.value 
          : config.type === 'unicode' 
            ? config.value 
            : formData.code;
              ...
              <div className="flex items-center gap-3">
                {formData.icon && (
                  <div className="relative">
                    <CurrencyIcon 
                      currencyCode={formData.code || 'SAR'} 
                      size={48}
                ...
                <Button
                  ...
                  className="flex-1"
                >
                  {formData.icon ? 'تغيير الأيقونة' : 'اختر أيقونة'}
                </Button>
              </div>
              
              {showIconPicker && (
                <div className="mt-3 border rounded-lg p-4 bg-background">
                  <Label className="mb-3 block">اختر أيقونة العملة</Label>
                  <ScrollArea className="h-64">
                    <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-3">
                      {Object.entries(CURRENCY_ICON_CONFIG).map(([code, config]) => {
                        const isSelected = formData.icon === config.value ||
                          (config.type === 'unicode' && formData.icon === config.value) ||
                          (config.type === 'svg' && formData.icon === config.value);
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              // Store the icon URL or Unicode value based on type
                              const iconValue = config.type === 'svg' 
                                ? config.value 
                                : config.type === 'unicode' 
                                  ? config.value 
                            ...
                            className={`
                              relative p-2 rounded-lg border-2 transition-all hover:scale-110 
                  ...
                  <div className="mt-3 pt-3 border-t">
                    <Label className="text-xs text-muted-foreground">أو أدخل رابط أيقونة مخصص</Label>
                    <Input
                      type="url"
                      placeholder="https://example.com/icon.png"
                      value={formData.icon && !Object.values(CURRENCY_ICON_CONFIG).some(c => c.value === formData.icon) ? formData.icon : ''}
                      onChange={(e) => {
                        if (e.target.value) {
