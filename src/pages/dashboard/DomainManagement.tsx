...
export default function DomainManagement() {
  ...
  const [domain, setDomain] = useState<{ subdomain?: string; customDomain?: string; verified?: boolean } | null>(null);
  ...
  // Detect production domain from environment
  ...
  const prodDomain = window.location.hostname.includes(platformSecondaryDomain) 
    ? platformSecondaryDomain 
    : platformDomain;
    
  const defaultDomain = domain?.subdomain ? `${domain.subdomain}.${prodDomain}` : `mystore.${prodDomain}`;
  ...
  const handleUpdateSubdomain = async () => {
    ...
    setSuggestions([]);
    try {
      const result: any = await coreApi.updateCurrentUserTenant({ subdomain: newSubdomain });
