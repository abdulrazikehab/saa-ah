  ...
  const loadData = useCallback(async (showLoading = true) => {
    try {
      ...
      const [brandsData, categoriesData, productsData] = await Promise.all([
        ...
        coreApi.getCategories({ limit: 10000 } as any, true).catch(() => []), // Require auth for dashboard
        coreApi.getProducts({ limit: '10000', includeBrand: true, includeCategories: true } as any, true).catch(() => []), // Require auth for dashboard
      ]);
      ...
      console.log('[HierarchicalManager] Loaded data:', {
        brands: Array.isArray(brandsData) ? brandsData.length : 0,
        categories: categoriesList.length,
        products: uniqueProducts.length,
        firstProduct: uniqueProducts.length > 0 ? uniqueProducts[0] : null,
      });
