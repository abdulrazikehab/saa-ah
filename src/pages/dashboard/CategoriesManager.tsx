...
import { Download, Upload, Users, FolderOpen, Gift, Plus, Trash2, Tag, Pencil, Percent, Loader2, Store, ChevronLeft, ChevronRight } from 'lucide-react';
...
const PaginationControl = ({ page, totalPages, onPageChange, t }: { page: number, totalPages: number, onPageChange: (p: number) => void, t: any }) => (
...
export default function CategoriesManager() {
  ...
  // Categories state
  const [brands, setBrands] = useState<{id: string; name: string; nameAr?: string; code?: string; logo?: string}[]>([]);
  ...
  const [editingBrand, setEditingBrand] = useState<{id: string; name: string; nameAr?: string; code?: string; logo?: string} | null>(null);
  const [brandFormData, setBrandFormData] = useState({ name: '', nameAr: '', code: '', logo: '' });

  const handleOpenBrandDialog = (brand?: typeof editingBrand) => {
    if (brand) {
      ...
      setBrandFormData({ name: '', nameAr: '', code: '', logo: '' });
    }
  ...
  const handleSaveBrand = async () => {
    ...
    try {
      setSavingBrand(true);
      if (editingBrand) {
        await coreApi.put(`/brands/${editingBrand.id}`, brandFormData, { requireAuth: true });
        toast({ title: t('common.success'), description: 'Brand updated successfully' });
      } else {
        await coreApi.post('/brands', brandFormData, { requireAuth: true });
        toast({ title: t('common.success'), description: 'Brand created successfully' });
      }
      setBrandDialogOpen(false);
      loadData(); // Refresh list
    } catch (error: any) {
      console.error('Failed to save brand', error);
      toast({ title: t('common.error'), description: error.message || 'Failed to save brand', variant: 'destructive' });
    } finally {
  ...
  const handleDeleteBrand = async (id: string) => {
    ...
    try {
        ...
        toast({ title: t('common.success'), description: 'Brand deleted successfully' });
        loadData();
    } catch (error: any) {
        toast({ title: t('common.error'), description: error.message || 'Failed to delete brand', variant: 'destructive' });
    }
  };
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    try {
      ...
      toast({ title: t('common.error'), description: error.message || 'Failed to delete category', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteAllBrands = async () => {
    if (!confirm('هل أنت متأكد من حذف جميع العلامات التجارية؟')) return;
    try {
      setLoading(true);
      ...
      toast({ title: t('common.success'), description: `تم حذف ${deleted} علامة تجارية بنجاح` });
      loadData();
    } catch (error: any) {
      toast({ title: t('common.error'), description: error.message, variant: 'destructive' });
    } finally {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [brandsData, categoriesData, productsData] = await Promise.all([
        coreApi.get(`/brands?page=${brandsPage}&limit=${ITEMS_PER_PAGE}`).catch(() => ({ data: [], meta: { totalPages: 1 } })),
        coreApi.getCategories({ page: categoriesPage, limit: ITEMS_PER_PAGE }),
        coreApi.getProducts({ limit: '1000' } as any).catch(() => []),
      ]);
      
      // Process Brands
      if (brandsData && typeof brandsData === 'object' && 'data' in brandsData) {
        setBrands(brandsData.data || []);
        setBrandsTotalPages((brandsData.meta && brandsData.meta.totalPages) || 1);
      } else {
  ...
  }, [t, toast, brandsPage, categoriesPage]);
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-4 mb-6">
          ...
          <TabsTrigger value="brands" className="gap-2">
            ...
            {t('dashboard.categories.tabs.brands', 'العلامات التجارية')}
          </TabsTrigger>
        </TabsList>

        <TabsContent value="brands">
          <div className="flex justify-between items-center gap-2 mb-4">
            <h2 className="text-xl font-semibold">{t('dashboard.brands.title', 'إدارة العلامات التجارية')}</h2>
            <div className="flex gap-2">
            <Button variant="destructive" onClick={handleDeleteAllBrands} className="gap-2" disabled={brands.length === 0}>
              <Trash2 className="h-4 w-4" />
              حذف الكل
            </Button>
            <Button onClick={() => handleOpenBrandDialog()} className="gap-2">
              <Plus className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />
              {t('dashboard.brands.addBrand', 'إضافة علامة تجارية')}
            </Button>
            </div>
          </div>
          <Card>
             ...
             <Table>
                <TableHeader>
                   <TableRow className="bg-muted/50">
                     <TableHead className="text-start font-semibold p-4">الاسم</TableHead>
                     <TableHead className="text-start font-semibold">الاسم (عربي)</TableHead>
                   ...
                   {brands.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={4} className="text-center py-12 text-muted-foreground">
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="text-lg">لا توجد علامات تجارية</p>
                            <p className="text-sm">ابدأ بإضافة علامة تجارية جديدة</p>
                          </div>
                        </TableCell>
                      </TableRow>
                   ) : (
                     brands.map(brand => (
                      <TableRow key={brand.id} className="hover:bg-muted/30">
                         <TableCell className="font-medium text-start p-4">
                            <div className="flex items-center gap-2">
                               {brand.logo ? <img src={brand.logo} className="h-8 w-8 object-contain rounded-md border bg-white p-0.5" alt={brand.name} /> : <div className="h-8 w-8 rounded-md bg-muted flex items-center justify-center"><Store className="h-4 w-4 text-muted-foreground" /></div>}
                               <span>{brand.name}</span>
                            </div>
                         </TableCell>
                         <TableCell className="text-start">{brand.nameAr || '-'}</TableCell>
                         <TableCell className="text-center"><Badge variant="secondary">{brand.code || '-'}</Badge></TableCell>
                         <TableCell className="text-center">
                            <div className="flex items-center justify-center gap-2">
                               <Button variant="ghost" size="icon" onClick={() => handleOpenBrandDialog(brand)}><Pencil className="h-4 w-4 text-blue-500" /></Button>
                               <Button variant="ghost" size="icon" onClick={() => handleDeleteBrand(brand.id)} className="text-destructive hover:text-destructive"><Trash2 className="h-4 w-4" /></Button>
                            </div>
                         </TableCell>
                      </TableRow>
                   )))}
              ...
              <CardContent className="p-0">
                <div className="w-full max-w-full overflow-x-auto" dir={isRTL ? 'rtl' : 'ltr'}>
                  <Table style={{ minWidth: '700px' }}>
                    <TableHeader>
                      <TableRow className="bg-muted/50">
                         <TableHead className={`font-semibold sticky ${isRTL ? 'right-0' : 'left-0'} bg-background z-20 ${isRTL ? 'border-l' : 'border-r'} shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] text-start`}>الاسم</TableHead>
                         <TableHead className={`font-semibold text-start`}>الاسم (عربي)</TableHead>
                         <TableHead className={`font-semibold text-start`}>الوصف</TableHead>
                        ...
                        categories.map((category) => (
                          <TableRow key={category.id} className="hover:bg-muted/30 transition-colors">
                             ...
                             <TableCell className={`py-4 text-start`}>
                               <span className="text-muted-foreground text-start">{category.nameAr || '-'}</span>
                             </TableCell>
                             <TableCell className={`py-4 text-start max-w-md`}>
                               <div className="truncate text-start" title={category.description || undefined}>
      ...
      <Dialog open={brandDialogOpen} onOpenChange={setBrandDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{editingBrand ? t('dashboard.brands.editBrand', 'تعديل علامة تجارية') : t('dashboard.brands.addBrand', 'إضافة علامة تجارية')}</DialogTitle>
            <DialogDescription>{t('dashboard.brands.desc', 'أدخل تفاصيل العلامة التجارية')}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label>الاسم (English) *</Label>
              <Input value={brandFormData.name} onChange={e => setBrandFormData({...brandFormData, name: e.target.value})} placeholder="Brand Name" />
            </div>
            <div className="grid gap-2">
              <Label>الاسم (عربي)</Label>
              <Input value={brandFormData.nameAr} onChange={e => setBrandFormData({...brandFormData, nameAr: e.target.value})} placeholder="اسم العلامة" style={{direction: 'rtl'}} />
            </div>
            <div className="grid gap-2">
              <Label>الكود (Code)</Label>
              <Input value={brandFormData.code} onChange={e => setBrandFormData({...brandFormData, code: e.target.value})} placeholder="CODE" />
            </div>
            <div className="grid gap-2">
              <Label>رابط الشعار (Logo URL)</Label>
              <Input value={brandFormData.logo} onChange={e => setBrandFormData({...brandFormData, logo: e.target.value})} placeholder="https://..." />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setBrandDialogOpen(false)}>إلغاء</Button>
