...
import { Download, Upload, Users, FolderOpen, Gift, Plus, Trash2, Tag, Pencil, Percent, Loader2, ChevronLeft, ChevronRight } from 'lucide-react';
...
interface ExportProduct { 
  sku?: string; 
  categories?: Array<{ categoryId?: string; id?: string; category?: { id: string } }>;
}

const ITEMS_PER_PAGE = 10;

const PaginationControl = ({ page, totalPages, onPageChange, t }: { page: number, totalPages: number, onPageChange: (p: number) => void, t: (key: string | string[], options?: string | object) => string }) => (
...
export default function CategoriesManager() {
  ...
  const [customerTiers, setCustomerTiers] = useState<CustomerTier[]>([
    { id: '1', name: 'فئة K', description: 'عملاء الفئة K - سعر خاص', color: '#FF6B6B', discountPercent: 25, productCategories: [], assignedCustomers: [] },
    { id: '2', name: 'فئة A', description: 'عملاء الفئة A - كبار العملاء', color: '#4ECDC4', discountPercent: 20, productCategories: [], assignedCustomers: [] },
    { id: '3', name: 'فئة B', description: 'عملاء الفئة B - عملاء متميزون', color: '#45B7D1', discountPercent: 15, productCategories: [], assignedCustomers: [] },
    { id: '4', name: 'فئة C', description: 'عملاء الفئة C - عملاء عاديون', color: '#96CEB4', discountPercent: 10, productCategories: [], assignedCustomers: [] },
  ]);
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    try {
      setLoading(true);
      console.log(`[CategoriesManager] Deleting category with ID: ${id}`);
      await coreApi.delete(`/categories/${id}`, { requireAuth: true });
      toast({ title: t('common.success'), description: 'Category deleted successfully' });
      loadData();
    } catch (error: any) {
      console.error('[CategoriesManager] Error deleting category:', error);
      ...
      if (error?.response?.status === 400) {
        ...
        if (message.includes('not found')) {
          toast({ 
            ...
            description: 'Category not found. Please refresh the page.', 
            variant: 'destructive' 
          });
          loadData(); // Reload to refresh the list
        } else if (message.includes('has products') || message.includes('subcategory')) {
          toast({ 
            title: t('common.error'), 
            description: message, 
            variant: 'destructive' 
  ...
  const handleDeleteAllCategories = async () => {
    if (!confirm('هل أنت متأكد من حذف جميع الفئات؟')) return;
    try {
      setLoading(true);
      ...
      // Rate-limited deletion with retry logic
      const deleteWithRetry = async (id: string, retries = 3): Promise<void> => {
      ...
      toast({ title: t('common.success'), description: `تم حذف ${deleted} فئة بنجاح` });
      loadData();
    } catch (error) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [categoriesData, productsData] = await Promise.all([
        coreApi.getCategories({ page: categoriesPage, limit: ITEMS_PER_PAGE }),
        coreApi.getProducts({ limit: 1000 } as Record<string, unknown>).catch(() => []),
      ]);

      // Process Categories
      let categoriesList: Array<{id: string; name: string; nameAr?: string; description?: string; slug?: string; image?: string; productCount?: number; parentId?: string; isActive?: boolean; sortOrder?: number; minQuantity?: number; maxQuantity?: number; enableSlider?: boolean; applySliderToAllProducts?: boolean}> = [];
      if (categoriesData && 'meta' in categoriesData) {
        categoriesList = categoriesData.categories;
        setCategoriesTotalPages(categoriesData.meta.totalPages || 1);
      } else {
        // Safe type assertion for legacy format
        const data = categoriesData as { categories?: typeof categoriesList; data?: typeof categoriesList } | typeof categoriesList;
      ...
      const parseBoolean = (val: string | boolean | undefined): boolean => {
        if (typeof val === 'boolean') return val;
        if (!val) return false;
        const s = String(val).toLowerCase();
        return s === 'true' || s === 'yes' || s === 'نعم' || s === '1';
      };
      ...
      setProducts(productsList.map((p: ProductItem) => {
        // Normalize categories
        let normalizedCategories: Array<{ categoryId?: string; id?: string; category?: { id: string } }> = [];
        if (p.categories && Array.isArray(p.categories)) {
          normalizedCategories = p.categories.map((cat: string | { categoryId?: string; id?: string; category?: { id: string } }) => {
        ...
        return {
          ...
          brandId: p.brandId || ((p as unknown as Record<string, { id: string }>)?.brand?.id), // Cast to unknown then Record to access brand property safely
      ...
      }) as Array<{id: string; name: string}>); // Type assertion to match state type
      ...
      setCategories([] as typeof categories); // Type assertion for empty array
    } finally {
      setLoading(false);
    }
  }, [t, toast, categoriesPage]);
    ...
    try {
      ...
      if (categoriesData && 'meta' in categoriesData && categoriesData.meta.totalPages > 1) {
        ...
        allPages.forEach((pageData) => {
          const pData = pageData as { categories?: typeof allCategories };
          if (pData && 'categories' in pData) {
            allCategories = [...allCategories, ...(pData.categories || [])];
          }
        });
      }

      // Fetch all products to get SKUs - fetch in batches if needed
      let allProducts: ExportProduct[] = [];
      try {
        const productsData = await coreApi.getProducts({ limit: 10000 } as Record<string, unknown>);
        
        // Handle paginated response
        if (productsData && typeof productsData === 'object' && 'data' in productsData) {
          const pData = productsData as { data: ExportProduct[]; meta?: { totalPages: number } };
          allProducts = pData.data || [];
          
          // If paginated, fetch all pages
          if (pData.meta && pData.meta.totalPages > 1) {
            const totalPages = pData.meta.totalPages;
            const allProductPages = await Promise.all(
              Array.from({ length: totalPages - 1 }, (_, i) => 
                coreApi.getProducts({ limit: 10000, page: i + 2 } as Record<string, unknown>)
              )
            );
            allProductPages.forEach((pageData) => {
              if (pageData && typeof pageData === 'object' && 'data' in pageData) {
                const pPageData = pageData as { data: ExportProduct[] };
                allProducts = [...allProducts, ...(pPageData.data || [])];
              } else if (Array.isArray(pageData)) {
                allProducts = [...allProducts, ...(pageData as ExportProduct[])];
              }
            });
          }
        } else if (Array.isArray(productsData)) {
          // Non-paginated array response
          allProducts = productsData as ExportProduct[];
        } else if (productsData && typeof productsData === 'object' && 'products' in productsData) {
          // Legacy format with safe guarding
          const legacyData = productsData as { products: ExportProduct[] };
          allProducts = legacyData.products || [];
        }
      } catch (error) {
        console.warn('Could not fetch all products for export:', error);
        // Use already loaded products as fallback with type mapping
        allProducts = products.map(p => ({
          sku: (p as any).sku || '',
          categories: (p as any).categories || []
        })) as ExportProduct[]; // Safe cast here since we map to the shape
      }

      // Create a map of category ID to product SKUs
      const categoryProductMap = new Map<string, string[]>();
      
      allProducts.forEach((product: ExportProduct) => {
        if (product && product.categories && Array.isArray(product.categories)) {
          product.categories.forEach((cat) => {
            const c = cat as { categoryId?: string; id?: string; category?: { id: string } };
            const categoryId = c.categoryId || c.id || c.category?.id;
            if (categoryId && product.sku) {
              if (!categoryProductMap.has(categoryId)) {
                categoryProductMap.set(categoryId, []);
              }
              const skus = categoryProductMap.get(categoryId);
              if (skus) skus.push(product.sku);
            }
          });
        }
      });
      ...
      const categoryMap = new Map<string, { id: string; name?: string; nameAr?: string; parentId?: string }>();
      ...
      const buildCategoryPath = (category: any, useArabic: boolean = false): string => {
        ...
        while (current) {
          const cat = current as { name?: string; nameAr?: string; parentId?: string };
          const name = useArabic ? ((cat.nameAr || cat.name) || '') : (cat.name || '');
          if (name) {
            path.unshift(name);
          }
          
          if (cat.parentId && categoryMap.has(cat.parentId)) {
            current = categoryMap.get(cat.parentId);
          } else {
            break;
          }
        }
      ...
      const exportData = allCategories.map(c => {
        ...
        const cat = c as { nameAr?: string; descriptionAr?: string; image?: string; icon?: string; isActive?: boolean; sortOrder?: number; minQuantity?: number; maxQuantity?: number; enableSlider?: boolean; applySliderToAllProducts?: boolean };
        return {
          'مسار الفئة': categoryPath, // Full path: Category > Subcategory > Subcategory...
          'الاسم': c.name || '',
          'الاسم (عربي)': cat.nameAr || '',
          'الوصف': c.description || '',
          'الوصف (عربي)': cat.descriptionAr || '',
          'الرابط': c.slug || '',
          'الصورة': cat.image || '',
          'الأيقونة': cat.icon || '',
          'نشط': cat.isActive !== undefined ? (cat.isActive ? 'نعم' : 'لا') : 'نعم',
          'ترتيب العرض': cat.sortOrder || 0,
          'الحد الأدنى للكمية': cat.minQuantity || '',
          'الحد الأقصى للكمية': cat.maxQuantity || '',
          'تفعيل السلايدر': cat.enableSlider !== undefined ? (cat.enableSlider ? 'نعم' : 'لا') : 'لا',
          'تطبيق السلايدر على جميع المنتجات': cat.applySliderToAllProducts !== undefined ? (cat.applySliderToAllProducts ? 'نعم' : 'لا') : 'لا',
      ...
      toast({
        variant: 'destructive',
        title: t('common.error'),
        description: err.message || 'Failed to export categories',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImportProgress({ current: 0, total: 0, currentItem: t('dashboard.categories.productCategories.importingCategoriesDesc', 'Reading file...') });
      
      const data = await file.arrayBuffer();
      setImportProgress({ current: 10, total: 100, currentItem: t('dashboard.categories.productCategories.analyzingData', 'Analyzing data...') });
      ...
      const getField = (row: Record<string, any>, arabicKey: string, englishKey: string): string | undefined => {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        try {
              ...
              for (let j = 0; j < pathParts.length - 1; j++) {
                ...
                if (!foundCategoryId) {
                  try {
                    ...
                    let errorMsg = error?.response?.data?.message || error?.data?.message || error?.message || 'خطأ غير معروف';
                    ...
                    collectedErrors.push({
                      ...
                      itemName: itemName,
                      error: `فشل إنشاء الفئة الأب "${pathPart}": ${errorMsg}`
                    });
                    // Break out of path building loop, but continue to next category
                    parentId = undefined;
                    break;
                  }
                }
          ...
          else if (errorLower.includes('tenant') || errorLower.includes('market')) errorColumn = 'General';
        ...
        <TabsContent value="categories">
          <div className="flex justify-between items-center gap-2 mb-4">
            ...
            <div className="flex gap-2">
              <Button variant="destructive" onClick={handleDeleteAllCategories} className="gap-2" disabled={categories.length === 0}>
          ...
          {loading ? (
            ...
            <Card>
              <CardContent className="p-0">
                <div className="w-full max-w-full overflow-x-auto" dir={isRTL ? 'rtl' : 'ltr'}>
                  <Table style={{ minWidth: '700px' }}>
                    <TableHeader>
                      <TableRow className="bg-muted/50">
                         <TableHead className={`font-semibold sticky ${isRTL ? 'right-0' : 'left-0'} bg-background z-20 ${isRTL ? 'border-l' : 'border-r'} shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] text-start`}>
                           {t('dashboard.categories.productCategories.categoryName')}
                         </TableHead>
                         <TableHead className={`font-semibold text-start`}>
                           {t('dashboard.categories.productCategories.description')}
                         </TableHead>
                        <TableHead className="font-semibold text-center">عدد المنتجات</TableHead>
                        <TableHead className={`font-semibold text-center sticky ${isRTL ? 'left-0' : 'right-0'} bg-background z-20 ${isRTL ? 'border-r' : 'border-l'} shadow-[-2px_0_4px_-2px_rgba(0,0,0,0.1)]`}>الإجراءات</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {categories.length === 0 ? (
                        ...
                        categories.map((category) => (
                          <TableRow key={category.id} className="hover:bg-muted/30 transition-colors">
                             <TableCell className={`font-medium py-4 sticky ${isRTL ? 'right-0' : 'left-0'} bg-background z-10 ${isRTL ? 'border-l' : 'border-r'} shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] text-start`}>
                             ...
                             <TableCell className={`py-4 text-start max-w-md`}>
                               <div className="truncate text-start" title={isRTL ? (category.descriptionAr || category.description) : category.description}>
                            ...
                            <TableCell className={`py-4 sticky ${isRTL ? 'left-0' : 'right-0'} bg-background z-10 ${isRTL ? 'border-r' : 'border-l'} shadow-[-2px_0_4px_-2px_rgba(0,0,0,0.1)]`}>
          ...
          {categoriesTotalPages > 1 && !loading && (
             <PaginationControl 
               page={categoriesPage} 
