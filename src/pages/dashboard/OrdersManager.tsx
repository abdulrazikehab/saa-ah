...
export default function OrdersManager() {
  ...
  const handleApproveOrder = async (orderId: string) => {
    try {
      ...
      toast({
        title: t('dashboard.orders.approveSuccess') || 'تم الموافقة على الطلب بنجاح',
        description: t('dashboard.orders.orderApproved') || 'تم تغيير حالة الطلب إلى موافق عليه',
      });
      loadOrders();
    } catch (error: unknown) {
      const err = error as { status?: number; message?: string };
      ...
      toast({
        title: t('dashboard.orders.approveError') || 'فشل الموافقة على الطلب',
        description: err?.message || t('dashboard.orders.updateStatusError'),
        variant: 'destructive',
      });
      }
    } finally {
  ...
  const handleRejectOrder = async () => {
    ...
    try {
      ...
      await coreApi.rejectOrder(selectedOrderForReject.id, orderRejectReason);
      toast({
        title: t('dashboard.orders.rejectSuccess') || 'تم رفض الطلب بنجاح',
        description: t('dashboard.orders.orderRejected') || 'تم تغيير حالة الطلب إلى مرفوض',
      });
      ...
      toast({
        title: t('dashboard.orders.rejectError') || 'فشل رفض الطلب',
        description: err?.message || t('dashboard.orders.updateStatusError'),
        variant: 'destructive',
      });
    } finally {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; className: string; icon: React.ElementType }> = {
      PENDING: { label: t('dashboard.orders.pending'), className: 'bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-500/20', icon: Clock },
      PROCESSING: { label: t('dashboard.orders.processing'), className: 'bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20', icon: Package },
      SHIPPED: { label: t('dashboard.orders.shipped'), className: 'bg-purple-500/10 text-purple-700 dark:text-purple-400 border-purple-500/20', icon: Truck },
      DELIVERED: { label: t('dashboard.orders.delivered'), className: 'bg-green-500/10 text-green-700 dark:text-green-400 border-green-500/20', icon: CheckCircle },
      APPROVED: { label: t('dashboard.orders.approved'), className: 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20', icon: CheckCircle },
      REJECTED: { label: t('dashboard.orders.rejected'), className: 'bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20', icon: XCircle },
      CANCELLED: { label: t('dashboard.orders.cancelled'), className: 'bg-gray-500/10 text-gray-700 dark:text-gray-400 border-gray-500/20', icon: XCircle },
    };
    ...
    return (
      <Badge variant="outline" className={`${config.className} gap-1 whitespace-nowrap`}>
            ...
            <Card className="border-s-4 border-s-blue-600 min-w-0">
              <CardContent className="p-4 md:pt-6">
                <div className="text-center">
                  ...
                  <p className="text-2xl md:text-3xl font-bold mt-1 md:mt-2">{stats.processing}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-s-4 border-s-purple-500 min-w-0">
              <CardContent className="p-4 md:pt-6">
                <div className="text-center">
                  ...
                  <p className="text-2xl md:text-3xl font-bold mt-1 md:mt-2">{stats.shipped}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-s-4 border-s-green-500 min-w-0 col-span-2 sm:col-span-1">
              <CardContent className="p-4 md:pt-6">
                <div className="text-center">
                  ...
                  <p className="text-2xl md:text-3xl font-bold mt-1 md:mt-2">{stats.delivered}</p>
                </div>
              </CardContent>
            </Card>
          </div>
