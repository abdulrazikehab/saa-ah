...
export default function ProductsManager() {
  ...
  const [currencies, setCurrencies] = useState<Array<{ id: string; code: string; name: string; nameAr?: string; symbol: string; exchangeRate: number; isDefault?: boolean }>>([]);
  ...
  const loadData = async () => {
    try {
      ...
      const [productsResponse, categoriesData, unitsData, brandsData, suppliersData, currenciesData] = await Promise.all([
      ...
      console.log('📦 Suppliers Data Raw:', suppliersData);
      const validatedSuppliers = validateArray<any>(suppliersData);
      console.log('📦 Validated Suppliers:', validatedSuppliers.length);
      setSuppliers(validatedSuppliers);
      
      setCurrencies(validateArray<any>(currenciesData));
      ...
      const mappedProducts: Product[] = rawProducts.map((p: ProductApiResponse): Product => {
        ...
        return {
          ...
          min: p.min,
          max: p.max,
          enableSlider: p.enableSlider,
          sliderStep: p.sliderStep,
          sliderStepMode: p.sliderStepMode,
          featuredPriceIncrease: p.featuredPriceIncrease,
          featuredPriceCurrency: p.featuredPriceCurrency,
          coinsNumber: p.coinsNumber,
          priceExceed: p.priceExceed
        };
      });
  ...
  const openEditDialog = (product: Product) => {
    ...
    setWizardInitialData({
      ...
      enableSlider: product.enableSlider || false,
      featuredPriceIncrease: product.featuredPriceIncrease?.toString() || '',
      featuredPriceCurrency: product.featuredPriceCurrency || 'SAR',
      coinsPerUnit: product.coinsNumber?.toString() || '',
      priceExceed: product.priceExceed || false,
      sliderStep: product.sliderStep?.toString() || '1',
      sliderStepMode: product.sliderStepMode || 'QUANTITY',
    });
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchQuery, filterStatus, filterCategory, activeTab]);
  
  // Reload data when pagination changes
  useEffect(() => {
    loadData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPage, itemsPerPage, debouncedSearchQuery, filterStatus, filterCategory, activeTab]);
  ...
  const handleSaveProduct = async (data: ProductFormData, images: string[]) => {
    try {
      ...
      const productData = {
        ...
        productCode: data.productCode ? data.productCode.trim() : (data.productCode === '' ? null : undefined),
        ...
        sliderStep: data.sliderStep ? parseInt(data.sliderStep) : undefined,
        sliderStepMode: data.sliderStepMode || 'QUANTITY',
        featuredPriceIncrease: data.featuredPriceIncrease ? parseFloat(data.featuredPriceIncrease) : undefined,
        featuredPriceCurrency: data.featuredPriceCurrency || 'SAR',
        coinsNumber: data.coinsPerUnit ? parseInt(data.coinsPerUnit) : undefined,
  ...
  const handleDeleteProduct = async (id: string) => {
    if (!await confirm({
      ...
      description: t('dashboard.products.messages.deleteConfirm'),
  ...
  const handleBulkDelete = async () => {
    ...
    if (!await confirm({
      title: t('dashboard.products.messages.confirmDelete', { count }),
      description: t('dashboard.products.messages.bulkDeleteConfirm'),
      confirmText: t('common.delete'),
      cancelText: t('common.cancel'),
      variant: 'destructive'
    })) return;
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-1">{formatNumber(stats.active)}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-1">{formatNumber(stats.lowStock)}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-1">{formatNumber(stats.outOfStock)}</p>
              </div>
        ...
        <TabsList className="mb-4">
          ...
          <TabsTrigger value="low-stock" className="gap-2">
            ...
            {activeTab !== 'low-stock' && stats.lowStock > 0 && (
              <Badge variant="secondary" className="h-5 px-1.5 min-w-[1.25rem]">{formatNumber(stats.lowStock)}</Badge>
            )}
          </TabsTrigger>
          <TabsTrigger value="out-of-stock" className="gap-2">
            ...
            {activeTab !== 'out-of-stock' && stats.outOfStock > 0 && (
              <Badge variant="secondary" className="h-5 px-1.5 min-w-[1.25rem]">{formatNumber(stats.outOfStock)}</Badge>
            )}
          </TabsTrigger>
        </TabsList>
            ...
            <Table className="w-full table-auto" style={{ minWidth: '900px' }}>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className={`font-semibold text-start w-32 whitespace-nowrap`}>Product Code</TableHead>
                  ...
                  >
                    <TableCell className={`sticky ${isRTL ? 'right-0' : 'left-0'} bg-background z-10 ${isRTL ? 'border-l' : 'border-r'} shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)]`} onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={`Select ${isRTL ? (product.nameAr || product.name) : product.name}`}
                      />
                    </TableCell>
                    <TableCell
                    ...
                    >
                      <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                        <div className="w-14 h-14 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 rounded-xl flex items-center justify-center overflow-hidden border-2 border-transparent transition-all duration-300 group-hover:border-primary group-hover:shadow-lg group-hover:scale-110">
                          {product.images?.[0] ? (
                            <img src={product.images[0]} alt={isRTL ? (product.nameAr || product.name) : product.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
                          ) : (
                            <ImageIcon className="h-7 w-7 text-gray-400 transition-all duration-300 group-hover:text-primary" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0 text-start">
                          <p className="font-semibold text-gray-900 dark:text-white group-hover:text-primary transition-colors duration-300 break-words">
                            {isRTL ? (product.nameAr || product.name) : product.name}
                          </p>
                          <p className="text-sm text-gray-500 dark:text-gray-400 break-words">
                            {isRTL ? (product.category?.nameAr || product.category?.name || 'بدون فئة') : (product.category?.name || 'No Category')}
                          </p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className={`font-mono text-sm text-gray-600 dark:text-gray-300 text-start break-all`}>{product.sku}</TableCell>
                    <TableCell className={`font-mono text-sm text-gray-600 dark:text-gray-300 text-start break-all`}>{product.productCode || '-'}</TableCell>
                    ...
                    <TableCell className="text-start">
                      <div className="text-start">
                        <p className="font-bold text-gray-900 dark:text-white">{formatCurrency(product.price || 0)}</p>
                        {product.compareAtPrice && (
                          <p className="text-xs text-gray-500 line-through">{formatCurrency(product.compareAtPrice)}</p>
                        )}
                      </div>
                    </TableCell>
                    <TableCell className="text-start">
                      <div className={`flex items-center gap-2 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                        <span className="font-medium text-gray-900 dark:text-white">{formatNumber(product.stock)}</span>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-baseline gap-3">
                        <span className="text-4xl font-bold text-primary">
                          {formatCurrency(viewingProduct.price || 0).replace(/[^\d.,]/g, '')}
                        </span>
                        <span className="text-xl text-gray-600 dark:text-gray-400">{t('common.currency', 'SAR')}</span>
                      </div>
                      {viewingProduct.compareAtPrice && (
                        <div className="mt-2 flex items-center gap-2">
                          <span className="text-lg text-gray-500 line-through">
                            {formatCurrency(viewingProduct.compareAtPrice)}
                          </span>
                          <Badge className="bg-red-500 text-white">
                            {t('dashboard.products.discount')} {formatNumber(Math.round((1 - viewingProduct.price / viewingProduct.compareAtPrice) * 100))}%
                          </Badge>
                        </div>
                      )}
                      {viewingProduct.cost && (
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                          {t('dashboard.products.cost')}: {formatCurrency(viewingProduct.cost)}
                        </p>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-3xl font-bold">{formatNumber(viewingProduct.stock)}</span>
                        <span className="text-gray-500">{t('dashboard.products.unitAvailable', 'وحدة متوفرة')}</span>
                      </div>
                      <div className="mt-3 pt-3 border-t">
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {t('dashboard.products.alertThreshold', 'حد التنبيه')}: {formatNumber(viewingProduct.lowStockThreshold)} {t('dashboard.products.unit', 'وحدة')}
                        </p>
                      </div>
                    </CardContent>
                  </Card>
