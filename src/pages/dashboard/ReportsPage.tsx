...
export default function ReportsPage() {
  ...
  const getDateRanges = useCallback((range: string) => {
    const now = new Date();
  ...
  const processOrdersForChart = useCallback((orders: { createdAt: string; total: number | string }[]) => {
    ...
    const diffDays = Math.min(Math.ceil(diffTime / (1000 * 60 * 60 * 24)), 90); // Limit to 90 days for chart readability
  ...
  const loadReportData = useCallback(async () => {
    try {
      ...
      const minDate = previousStart < currentStart ? previousStart : currentStart;
      const maxDate = previousEnd > currentEnd ? previousEnd : currentEnd;

      // Get orders for both periods with higher limit
      const [allOrdersData, statsData, productsData, customersData, paymentsData] = await Promise.all([
        ...
        reportService.getCustomerReport().catch(() => []),
        reportService.getPaymentReport().catch(() => [])
      ]);

      const allOrdersDataRaw = allOrdersData as unknown as { orders?: ReportOrder[]; data?: ReportOrder[] } | ReportOrder[];
      const allOrders = (Array.isArray(allOrdersDataRaw) ? allOrdersDataRaw : (allOrdersDataRaw.orders || allOrdersDataRaw.data || [])) as ReportOrder[];
      
      const statsDataRaw = statsData as { orderCount: number; revenue: number; productCount: number; customerCount: number };
      ...
      const currentRevenue = currentOrders.reduce((sum: number, order) => sum + Number(order.total || 0), 0);
      ...
      const previousRevenue = previousOrders.reduce((sum: number, order) => sum + Number(order.total || 0), 0);
      ...
      setPreviousStats({
        ...
        productCount: statsDataRaw.productCount, // Products don't have date-based comparison
        customerCount: statsDataRaw.customerCount, // Customers don't have date-based comparison
      });
  ...
  }, [dateRange, productsPage, productsPerPage, getDateRanges, processOrdersForChart]);
  ...
  useEffect(() => {
    loadReportData();
  }, [loadReportData, dateRange, customStartDate, customEndDate]);
  ...
  }, [dateRange, originalSalesData, salesData, productReport, customerReport, paymentReport, getDateRanges]);
        ...
        <div className="flex gap-2">
          <Select value={dateRange} onValueChange={setDateRange}>
            ...
            <SelectContent>
              ...
              <SelectItem value="custom">{t('dashboard.reports.customRange', 'فترة مخصصة')}</SelectItem>
            </SelectContent>
          </Select>
          {dateRange === 'custom' && (
            <div className="flex items-center gap-2 bg-background border rounded-md px-2 py-1 shadow-sm">
              <Input
                ...
                className="border-0 focus-visible:ring-0 w-[130px] p-0 h-8"
              />
        ...
        <TabsContent value="products" className="space-y-6">
          <Card>
            <CardHeader className="border-b">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>{t('dashboard.reports.productPerformance')} ({formatNumber(productsTotalItems)})</CardTitle>
                      ...
                      <div key={product.id} className="flex items-center p-3 rounded-lg hover:bg-muted/50 transition-colors">
                        <div className="flex-1 space-y-1">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {formatNumber(product.salesCount)} {t('dashboard.reports.sales')} | SKU: {product.sku || 'N/A'}
                          </p>
                        </div>
                ...
                {customerReport.map((customer) => (
                  <div key={customer.email} className="flex items-center">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-xs text-muted-foreground">{formatNumber(customer.orders)} {t('dashboard.reports.orders')}</div>
                    </div>
                  </div>
                ))}
