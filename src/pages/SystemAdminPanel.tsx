...
import { getAdminApiKeySync, initializeAdminApiKey, clearAdminApiKeyCache, setAdminApiKeyCache } from '@/lib/admin-config';
...
export default function SystemAdminPanel() {
  ...
  // PRIORITY: Use environment variable first, then try to fetch from backend
  useEffect(() => {
    // First, check environment variable (primary source for login)
    const envKey = import.meta.env.VITE_ADMIN_API_KEY;
    if (envKey) {
      setAdminPassword(envKey);
      console.log('Admin API Key initialized from environment:', `${envKey.substring(0, 10)}... (length: ${envKey.length})`);
    } else {
      // Try to fetch from backend (will fail if no env key, but that's okay)
      initializeAdminApiKey().then(() => {
        const apiKey = getAdminApiKeySync();
        if (apiKey) {
          setAdminPassword(apiKey);
          console.log('Admin API Key initialized from backend:', `${apiKey.substring(0, 10)}... (length: ${apiKey.length})`);
        } else {
          console.warn('⚠️ VITE_ADMIN_API_KEY is not set in .env file or dev server needs restart');
        }
      }).catch((error) => {
        console.warn('⚠️ Could not fetch admin API key from backend (this is normal if env var is not set):', error);
      });
    }
  }, []);
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // PRIORITY: Use environment variable first (since we can't fetch from backend without auth)
    // This is the primary source for login authentication
    const envKey = import.meta.env.VITE_ADMIN_API_KEY;
    const adminPasswordToCheck = envKey || ADMIN_PASSWORD;
    
    // Debug logging (remove in production)
    console.log('Login attempt:', {
      passwordLength: trimmedPassword.length,
      expectedLength: adminPasswordToCheck?.length || 0,
      hasEnvKey: !!envKey,
      hasCachedKey: !!ADMIN_PASSWORD,
    });
    
    if (!adminPasswordToCheck) {
      toast({
        variant: 'destructive',
        title: `❌ ${t.accessDenied}`,
        description: language === 'ar' 
          ? 'مفتاح API غير معين. يرجى تعيين VITE_ADMIN_API_KEY في ملف .env وإعادة تشغيل الخادم.'
          : 'API key not set. Please set VITE_ADMIN_API_KEY in .env file and restart the dev server.',
      });
      return;
    }
    
    // Compare passwords (case-sensitive, exact match)
    if (trimmedPassword === adminPasswordToCheck) {
      setIsAuthenticated(true);
      ...
      if (envKey && envKey !== ADMIN_PASSWORD) {
        setAdminPassword(envKey);
      }
      // Cache the admin API key so other components can use it
      setAdminApiKeyCache(adminPasswordToCheck);
      toast({
      ...
      const hasEnvKey = !!import.meta.env.VITE_ADMIN_API_KEY;
      const errorMessage = language === 'ar'
        ? `كلمة مرور غير صحيحة. المطلوب: ${adminPasswordToCheck.length} حرف. ${hasEnvKey ? 'تأكد من تطابق VITE_ADMIN_API_KEY في ملف .env مع المفتاح في قاعدة البيانات.' : 'يرجى تعيين VITE_ADMIN_API_KEY في ملف .env.'}`
        : `Incorrect password. Required: ${adminPasswordToCheck.length} characters. ${hasEnvKey ? 'Make sure VITE_ADMIN_API_KEY in .env matches the key in the database.' : 'Please set VITE_ADMIN_API_KEY in .env file.'}`;
