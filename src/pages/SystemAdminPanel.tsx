...
import { 
  ...
  LayoutDashboard, Bot, Fingerprint, Eye, Ban, MapPin, Monitor, Trash2, FileText, ChevronLeft, ChevronRight, Key, UserPlus, Settings, Loader2, Cloud, MessageSquare, Building2,
  XCircle, UserCheck, Package, ShoppingCart, BarChart3, RefreshCw, LucideIcon
} from 'lucide-react';
import { useConfirm } from '@/contexts/ConfirmationContext';
import { useConfirm as useConfirmContext } from '@/contexts/ConfirmationContext';
...
import SupplierProductsManager from './master-dashboard/SupplierProductsManager';
import VersionsManager from './master-dashboard/VersionsManager';


import { APP_VERSION } from '../version';
import { clearAdminApiKeyCache, setAdminApiKeyCache, initializeAdminApiKey, getAdminApiKeySync } from '@/lib/admin-config';
...
export default function SystemAdminPanel() {
  ...
  // Force update check
  useEffect(() => {
    console.log('SystemAdminPanel mounted - v2');
  }, []);
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'logs' | 'management' | 'overview' | 'tenants' | 'gateways' | 'partners' | 'plans' | 'features' | 'gifts' | 'database' | 'ai' | 'customers' | 'analytics' | 'users' | 'api-keys' | 'pages' | 'limits' | 'permissions' | 'cloudinary' | 'complaints' | 'platform-settings' | 'transactions' | 'versions' | 'supplier-products'>('dashboard');
  ...
  const toggleLanguage = () => {
    ...
    // Use centralized setLanguage function
  ...
  // Admin API key - initialized from environment variable
  // Admin API key - manual entry required
  const [ADMIN_PASSWORD, setAdminPassword] = useState('');
  const [isRestoringSession, setIsRestoringSession] = useState(true);

  // Auto-init removed to enforce manual entry

  // Normalize any log shape from backend/auth service so UI is robust
  const normalizeLogs = useCallback((raw: unknown, type: 'security' | 'audit' | 'error') => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data = raw as any;
    const list = (data?.logs) || (data?.data?.logs) || (data?.data) || data || [];
    if (!Array.isArray(list)) return [];
    return list.map((log: Record<string, unknown>, index: number) => {
      const id = (log.id as string) || (log._id as string) || `${type}-${index}-${(log.createdAt as string) || Date.now()}`;
      const severity = (log.severity as string) || (log.level as string) || (type === 'security' ? 'LOW' : type === 'error' ? 'HIGH' : undefined);
      const action = (log.action as string) || (log.event as string) || (log.type as string) || (type === 'error' ? 'ERROR' : 'UNKNOWN');
      // Handle details from different sources (activityLog uses details object, auditLog uses details string)
      let details = '';
      if (log.details) {
        if (typeof log.details === 'string') {
          details = log.details;
        } else if (typeof log.details === 'object' && (log.details as { message?: string }).message) {
          details = (log.details as { message?: string }).message || '';
        } else if (typeof log.details === 'object') {
          details = JSON.stringify(log.details);
        }
      }
      details = details || (log.message as string) || (log.description as string) || ((log.metadata as { message?: string })?.message) || '';
      
      // Extract URL from different sources
      let url = '';
      if (log.details && typeof log.details === 'object' && (log.details as { url?: string }).url) {
        url = (log.details as { url?: string }).url || '';
      } else if (log.metadata) {
        if (typeof log.metadata === 'object' && (log.metadata as { url?: string }).url) {
          url = (log.metadata as { url?: string }).url || '';
      ...
      if (!url && log.action && (log.action as string).includes(' ')) {
        const parts = (log.action as string).split(' ');
        if (parts.length >= 2) {
          url = parts.slice(1).join(' ');
        }
      }
      
      const createdAt = (log.createdAt as string) || (log.timestamp as string) || new Date().toISOString();
      
      // Handle user info from different sources (activityLog uses actorId, auditLog uses userId)
      const userEmail = (log.user as { email?: string })?.email || (log.userEmail as string) || (log.email as string) || ((log.metadata as { userEmail?: string })?.userEmail);
      const userName = (log.user as { name?: string })?.name || (log.userName as string) || (log.name as string) || ((log.metadata as { userName?: string })?.userName);
      
      const tenantName = (log.tenant as { name?: string })?.name || (log.tenantName as string) || (log.storeName as string) || (log.shopName as string);
      const resourceType = (log.resourceType as string) || (log.entity as string) || (log.model as string) || ((log.metadata as { resourceType?: string })?.resourceType) || (type === 'error' ? 'SYSTEM' : '');
      const resourceId = (log.resourceId as string) || (log.entityId as string) || (log.targetId as string) || '';
      return {
        ...log,
        id,
        severity,
        ...
        resourceType,
        resourceId,
        user: userEmail ? { email: userEmail, name: userName } : (log.user as { email: string; name?: string } || { email: 'System' }),
        tenant: tenantName ? { name: tenantName } : (log.tenant as { name: string }),
      } as AuditLog;
    });
  }, []);
  ...
  // Check for existing admin session on mount
  useEffect(() => {
    // Explicitly do NOT restore session to enforce login every time
    // and do not save user login
    setIsAuthenticated(false);
    setIsRestoringSession(false);
    
    // Ensure session storage is clean
  ...
  const fetchDashboardStats = useCallback(async () => {
    ...
    try {
      ...
      const response = await coreApi.get<DashboardStats>('/admin/master/overview', { requireAuth: true, adminApiKey: ADMIN_PASSWORD });
  ...
  const fetchSystemHealth = useCallback(async () => {
    if (!ADMIN_PASSWORD) return;
    try {
      const response = await coreApi.get<SystemHealth>('/admin/master/system-health', { requireAuth: true, adminApiKey: ADMIN_PASSWORD });
  ...
  const fetchSecurityEvents = useCallback(async () => {
    try {
      ...
      const [authData, coreData] = await Promise.allSettled([
        apiClient.get<AuditLog[]>(`${authUrl}/security-events`, { requireAuth: true, adminApiKey: ADMIN_PASSWORD, timeout: 20000 }),
        coreApi.get<AuditLog[]>('/admin/master/security-events', { requireAuth: true, adminApiKey: ADMIN_PASSWORD, timeout: 20000 }).catch(() => ({ logs: [] as AuditLog[] })), // Gracefully handle core backend errors
      ]);
  ...
  const fetchErrorLogs = useCallback(async () => {
    try {
      ...
      if (!ADMIN_PASSWORD) return;
      const [authData, coreData] = await Promise.allSettled([
        apiClient.get<AuditLog[]>(`${authUrl}/error-logs?limit=10000`, { requireAuth: true, adminApiKey: ADMIN_PASSWORD, timeout: 20000 }),
        coreApi.get<AuditLog[]>('/admin/master/error-logs?limit=10000', { requireAuth: true, adminApiKey: ADMIN_PASSWORD, timeout: 20000 }).catch(() => ({ logs: [] as AuditLog[] })),
      ]);
  ...
  const fetchAuditLogs = useCallback(async () => {
    try {
      ...
      if (!ADMIN_PASSWORD) return;
      const [authData, coreData] = await Promise.allSettled([
        apiClient.get<AuditLog[]>(`${authUrl}/audit-logs`, { requireAuth: true, adminApiKey: ADMIN_PASSWORD, timeout: 20000 }),
        coreApi.get<AuditLog[]>('/admin/master/audit-logs', { requireAuth: true, adminApiKey: ADMIN_PASSWORD, timeout: 20000 }).catch(() => ({ logs: [] as AuditLog[] })), // Gracefully handle core backend errors
      ]);
  ...
  }, [isAuthenticated, ADMIN_PASSWORD, fetchLogs, fetchDashboardStats, fetchSystemHealth]);
  ...
  const fetchCustomerFingerprints = useCallback(async () => {
    if (!ADMIN_PASSWORD) return;
    try {
      const data = await coreApi.get<CustomerFingerprint[]>('/admin/master/customers', { requireAuth: true, adminApiKey: ADMIN_PASSWORD });
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: err?.message || 'Could not load customer data',
      });
    }
  }, [toast, ADMIN_PASSWORD]);

  // Load customers when authenticated and tab is customers
  useEffect(() => {
    if (isAuthenticated && activeTab === 'customers' && ADMIN_PASSWORD) {
      fetchCustomerFingerprints();
    }
  }, [isAuthenticated, activeTab, ADMIN_PASSWORD, fetchCustomerFingerprints]);
  ...
  const fetchLimitsConfig = useCallback(async () => {
    if (!ADMIN_PASSWORD) return;
    setLoadingLimits(true);
    try {
      const config = await coreApi.get<{
  ...
  // Load limits configuration when authenticated and tab is limits
  useEffect(() => {
    if (isAuthenticated && activeTab === 'limits' && ADMIN_PASSWORD) {
      fetchLimitsConfig();
    }
  }, [isAuthenticated, activeTab, ADMIN_PASSWORD, fetchLimitsConfig]);
  ...
  const fetchDbStats = useCallback(async () => {
    ...
    try {
      // Use authUrl directly to include /auth prefix
      ...
      toast({
        variant: 'destructive',
        title: language === 'ar' ? 'خطأ' : 'Error',
        description: err?.message || (language === 'ar' ? 'فشل تحميل إحصائيات قاعدة البيانات' : 'Failed to load database statistics'),
      });
    } finally {
      setLoadingDbStats(false);
    }
  }, [ADMIN_PASSWORD, language, toast]);
  ...
  const fetchRateLimitConfig = useCallback(async () => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: language === 'ar' ? 'خطأ' : 'Error',
        description: err?.message || (language === 'ar' ? 'فشل تحميل إعدادات قيود المعدل' : 'Failed to load rate limit configuration'),
      });
    }
  }, [ADMIN_PASSWORD, language, toast]);
  ...
  // Fetch database stats and rate limit config when database tab is opened
  useEffect(() => {
    if (isAuthenticated && activeTab === 'database' && ADMIN_PASSWORD) {
      fetchDbStats();
      fetchRateLimitConfig();
    }
  }, [isAuthenticated, activeTab, ADMIN_PASSWORD, fetchDbStats, fetchRateLimitConfig]);

  const handleUpdateRateLimitConfig = async () => {
    const confirmed = await confirm({
      title: language === 'ar' ? 'تحديث الإعدادات' : 'Update Configuration',
      description: language === 'ar' 
        ? 'هل أنت متأكد من تحديث إعدادات قيود المعدل؟ سيتم تطبيق التغييرات فوراً.'
        : 'Are you sure you want to update rate limit configuration? Changes will be applied immediately.',
      confirmText: language === 'ar' ? 'تحديث' : 'Update',
    ...
    try {
      const response = await apiClient.fetch(`${apiClient.authUrl}/admin/rate-limit-config`, {
        method: 'PUT',
        body: JSON.stringify({
      ...
      toast({
        variant: 'destructive',
        title: '❌ Error',
        description: err?.message || (language === 'ar' ? 'فشل تحديث الإعدادات' : 'Failed to update configuration'),
      });
    } finally {
  ...
  const handleClearRateLimits = async (type?: string) => {
    ...
    const message = type 
      ? (language === 'ar' ? `مسح حدود المعدل لـ ${type}؟` : `Clear rate limits for ${type}?`)
      : (language === 'ar' ? 'مسح جميع حدود المعدل؟ سيقوم هذا بإزالة جميع القيود.' : 'Clear ALL rate limits? This will remove all rate limiting restrictions.');
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: '❌ Error',
        description: err?.message || 'Failed to clear rate limits',
      });
    } finally {
      setClearing(null);
    }
  ...
  const handleClearData = async (type: string) => {
    ...
    if (!await confirm({
      title: language === 'ar' ? 'تأكيد المسح' : 'Confirm Clear',
      description: language === 'ar' ? `مسح جميع ${type}؟ لا يمكن التراجع عن هذا الإجراء.` : `Clear all ${type}? This action cannot be undone.`,
      variant: 'destructive'
    })) return;
    ...
    const method = 'DELETE';
      ...
      // Use authUrl directly - do not strip /auth as the endpoints are mounted under the auth service
      // e.g. https://kawn.net/auth/admin/stats
      ...
      switch (type) {
        ...
        case 'systemSafe': {
          url = `/admin/master/clear-non-transaction-data`;
          break;
        }
        default:
      ...
      let result;
      if (type === 'systemSafe') {
        result = await coreApi.post(url, {}, {
          requireAuth: true,
          adminApiKey: ADMIN_PASSWORD,
        });
      } else {
        result = await apiClient.fetch(url, {
          method,
          requireAuth: true,
          adminApiKey: ADMIN_PASSWORD,
        });
  ...
  const handleResetDatabase = async () => {
    ...
    if (!await confirm({
      title: language === 'ar' ? 'إعادة ضبط قاعدة البيانات' : 'Reset Database',
      description: language === 'ar' 
        ? 'هل أنت متأكد؟ سيتم حذف جميع البيانات. هذا الإجراء لا يمكن التراجع عنه.' 
        : 'ARE YOU SURE? This will delete ALL data. This action cannot be undone.',
      variant: 'destructive',
      confirmText: language === 'ar' ? 'حذف الجميع' : 'DELETE ALL'
    })) return;

    if (!await confirm({
      title: language === 'ar' ? 'تحذير نهائي' : 'Final Warning',
      description: language === 'ar' ? 'حقاً؟ هذا تحذير نهائي!' : 'Really? Last warning!',
      variant: 'destructive',
      confirmText: language === 'ar' ? 'نعم، أنا متأكد' : 'Yes, I am sure'
    })) return;
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // Trim password input to avoid whitespace issues
    ...
    try {
      // Verify key against backend by making a lightweight call
      await coreApi.get('/admin/master/system-health', { 
        requireAuth: true, 
        adminApiKey: trimmedPassword,
        hideErrorToast: true 
      });

      // If successful (no 401), we allow access
      setIsAuthenticated(true);
      // Do not store admin session to prevent saving login
      // sessionStorage.setItem('systemAdminAuth', 'true');
      // sessionStorage.setItem('systemAdminKey', trimmedPassword);
      
      setAdminPassword(trimmedPassword);
      // Cache the admin API key
      setAdminApiKeyCache(trimmedPassword);
      ...
      // CRITICAL FIX: If server explicitly rejects key (401), DO NOT fallback to local check.
      // We only fallback if server is unreachable (status 0 or undefined)
      if (err?.status === 401) {
        toast({
          variant: 'destructive',
          title: `❌ ${t.accessDenied}`,
          description: language === 'ar' 
            ? 'مفتاح API غير صحيح. (رفض الخادم)'
            : 'Invalid API Key. (Server Rejected)',
        });
        setPassword('');
        return;
      }

      // Fallback to local check ONLY if backend is unreachable (e.g. server down)
      const envKey = import.meta.env.VITE_ADMIN_API_KEY;
      if (envKey && trimmedPassword === envKey) {
        setIsAuthenticated(true);
        // Do not store session
        // sessionStorage.setItem('systemAdminAuth', 'true');
        // sessionStorage.setItem('systemAdminKey', envKey);
        
        setAdminPassword(envKey);
        setAdminApiKeyCache(envKey);
        
        toast({
          variant: 'destructive', // Warn user they are in offline mode
          title: language === 'ar' ? '⚠️ وضع غير متصل' : '⚠️ Offline Mode',
          description: language === 'ar' 
            ? 'تعذر الاتصال بالخادم. تم تسجيل الدخول محلياً.'
            : 'Server unreachable. Logged in locally.',
        });
        return;
      }
      ...
      toast({
        ...
        title: `❌ ${t.accessDenied}`,
        description: language === 'ar' 
          ? 'مفتاح API غير صحيح أو تم رفض الوصول من قبل الخادم.'
          : 'Invalid API Key or access denied by server.',
      });
  ...
  const menuCategories = [
    ...
    {
      id: 'users',
      label: language === 'ar' ? 'المستخدمون والمستأجرون' : 'Users & Tenants',
      items: [
    ...
    {
      id: 'commerce',
      label: language === 'ar' ? 'التجارة' : 'Commerce',
      items: [
        ...
        { id: 'supplier-products', label: language === 'ar' ? 'منتجات الموردين' : 'Supplier Products', icon: Building2 },
      ]
    },
    {
      id: 'config',
      label: language === 'ar' ? 'الإعدادات' : 'Configuration',
      items: [
        ...
        { id: 'versions', label: language === 'ar' ? 'إدارة النسخ' : 'Versions', icon: Layers },
      ]
    },
    {
      id: 'monitoring',
  ...
  const theme = isDarkMode ? {
    ...
    accent: 'bg-gradient-to-r from-violet-600 via-indigo-600 to-blue-600 shadow-lg shadow-violet-500/20',
    accentHover: 'hover:shadow-violet-500/40 hover:scale-[1.02] transition-all',
    accentText: 'text-violet-400',
    accentBg: 'bg-violet-500/10',
    input: 'bg-slate-900/50 border-slate-700 text-white placeholder-slate-500 focus:bg-slate-900',
    focus: 'focus:border-violet-500 focus:ring-1 focus:ring-violet-500',
    sidebar: 'bg-slate-950/80 backdrop-blur-md',
    header: 'bg-slate-950/80 backdrop-blur-md',
    ...
    input: 'bg-white border-slate-200 text-slate-900 placeholder-slate-400 focus:bg-white',
    focus: 'focus:border-violet-500 focus:ring-1 focus:ring-violet-500',
    ...
    return (
      <div className={`min-h-screen ${theme.bg} flex items-center justify-center`}>
        <div className="flex flex-col items-center gap-4">
          ...
          <p className={theme.textMuted}>Restoring session...</p>
        </div>
      </div>
    );
  }
        ...
        {isSidebarOpen && (
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-4">
              {menuCategories.map((category) => (
                <div key={category.id} className="space-y-1">
                  <div className={`px-3 py-2 text-xs font-semibold uppercase tracking-wider ${theme.textDim}`}>
                    {category.label}
                  </div>
            ...
            <div className={`p-4 border-t ${theme.border}`}>
              ...
              <button
                onClick={async () => {
                  // Clear system admin specific auth
                  setIsAuthenticated(false);
                  setPassword('');
                  sessionStorage.removeItem('systemAdminAuth');
                  sessionStorage.removeItem('systemAdminKey');
                  ...
                  // Also clear global merchant/customer auth
                  try {
                    // We don't necessarily want to wait for the server logout call
                    // but we do want to clear local data
                    const auth = (window as unknown as { authContext?: { logout: () => void } }).authContext; // Fallback if useAuth not directly available
                    if (auth?.logout) {
                      auth.logout();
                    } else {
                      // Manual clear if context not reachable
                  ...
                  toast({
                    ...
                    description: language === 'ar' 
                      ? 'تم مسح مفاتيح الجلسة وتسجيل الخروج'
                      : 'Session keys cleared and logged out',
                  });
                }}
          ...
          >
                  ...
                  <div className={`${cardStyle} rounded-2xl p-6`}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <button 
                        ...
                        className={`p-4 ${theme.bg} border ${theme.border} rounded-xl hover:border-violet-500/50 transition-colors text-${isRTL ? 'right' : 'left'}`}
                      >
                        <Layers className="w-6 h-6 text-green-500 mb-2" />
                        <div className={`font-medium ${theme.text}`}>{language === 'ar' ? 'إدارة النسخ' : 'Manage Versions'}</div>
                        <div className={`text-xs ${theme.textMuted}`}>{language === 'ar' ? 'التحكم في القوائم' : 'Control sidebar tabs'}</div>
                      </button>
                    </div>
                  </div>
            ...
            {activeTab === 'database' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2 ${theme.accent} text-white rounded-lg ${theme.accentHover} transition-colors`}
                    >
                ...
                <div className="flex items-center justify-between">
                  <h3 className={`text-lg font-bold ${theme.text}`}>{t.dbStats}</h3>
                  <button
                ...
                <div className={`${cardStyle} p-6 rounded-2xl border border-red-500/20`}>
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="space-y-3 mb-8 pb-8 border-b border-gray-700/50">
                        <div className={`p-4 rounded-xl ${isDarkMode ? 'bg-blue-500/5' : 'bg-blue-50'} border border-blue-500/20 mb-4`}>
                          <p className="text-sm text-blue-400">
                            {language === 'ar' ? 'مسح بيانات النظام (آمن): سيؤدي هذا إلى مسح المنتجات والأصناف والسجلات، ولكن سيتم الحفاظ على المعاملات والعمليات المالية.' : 'Clear System Data (Safe): This will clear products, categories, and logs, but will PRESERVE transactions and financial records.'}
                          </p>
                        </div>
                        <button
                          onClick={() => handleClearData('systemSafe')}
                          disabled={clearing === 'systemSafe'}
                          className="w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50 font-bold shadow-lg shadow-blue-500/20"
                        >
                          {clearing === 'systemSafe' ? <RefreshCcw className="w-5 h-5 animate-spin" /> : <Layers className="w-5 h-5" />}
                          {language === 'ar' ? 'مسح بيانات النظام (آمن)' : 'Clear System Data (Safe)'}
                        </button>
                      </div>
            ...
            {activeTab === 'supplier-products' && (
              <SupplierProductsManager adminApiKey={ADMIN_PASSWORD} />
            )}
            {activeTab === 'overview' && <MasterOverview adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'tenants' && <TenantManagement adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'users' && <UserManagement adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'gateways' && <PaymentGatewayManager adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'partners' && <PartnerManager adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'plans' && <PlansManager adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'features' && <FeatureControlManager adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'ai' && <AiSettingsManager adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'gifts' && <UserGiftsManager adminApiKey={ADMIN_PASSWORD} />}
            {activeTab === 'api-keys' && (
              <div className="space-y-6">
                <ApiKeyManager adminApiKey={ADMIN_PASSWORD} />
            ...
            {activeTab === 'permissions' && (
              <PermissionsManager adminApiKey={ADMIN_PASSWORD} />
            )}

            {activeTab === 'cloudinary' && (
              <CloudinaryAccessManager adminApiKey={ADMIN_PASSWORD} />
            )}

            {activeTab === 'complaints' && (
              <ComplaintsManager adminApiKey={ADMIN_PASSWORD} />
            )}

            {activeTab === 'platform-settings' && (
              <PlatformSettings adminApiKey={ADMIN_PASSWORD} />
            )}

            {activeTab === 'transactions' && (
              <TransactionManager adminApiKey={ADMIN_PASSWORD} />
            )}

            {activeTab === 'versions' && (
              <VersionsManager adminApiKey={ADMIN_PASSWORD} />
            )}
                        ...
                        {filteredLogs.length === 0 ? (
                          ...
                          paginatedLogs.map((log) => (
                            <tr key={log.id} className={`${theme.cardHover} transition-colors`}>
                              ...
                              <td className={`px-2 py-4 ${theme.text}`}>
                                <div className="font-medium break-words">{log.user?.email || (log as { userEmail?: string }).userEmail || t.system}</div>
                                <div className={`text-xs ${theme.textDim} break-words`}>{log.user?.name || (log as { userName?: string }).userName || ''}</div>
                              </td>
                              <td className={`px-2 py-4 ${theme.text} break-words`}>
                                {log.tenant?.name || (log as { tenantName?: string }).tenantName || '-'}
                              </td>
                              <td className={`px-2 py-4 ${theme.textDim} whitespace-nowrap`}>
                                {new Date(log.createdAt).toLocaleString()}
                              </td>
...
function AdminApiKeySection({ adminPassword, onUpdate, theme, language, cardStyle }: {
  ...
  const handleUpdate = async () => {
    ...
    if (newApiKey.trim() === 'Koun2025Admin!' || newApiKey.trim().length < 20) {
      toast({
        variant: 'destructive',
        title: language === 'ar' ? 'خطأ' : 'Error',
        description: language === 'ar' 
