...
import { Page, UserProfile, Order } from '@/services/types';
...
export default function Profile() {
  ...
  const loadData = useCallback(async () => {
    try {
      // Try to fetch custom page for profile
      try {
        const profilePage = await coreApi.getPageBySlug('profile');
        if (profilePage && profilePage.isPublished && 
            profilePage.content?.sections && 
            Array.isArray(profilePage.content.sections) && 
            profilePage.content.sections.length > 0) {
          setCustomPage(profilePage);
        }
      } catch (e) {
  ...
  if (isNativeMode) return <MobileProfile />;
                  ...
                  <TableBody>
                    {orders.map((order) => (
                      <TableRow key={order.id}>
                        ...
                        <TableCell>{formatCurrency(order.totalAmount, settings?.currency || 'SAR')}</TableCell>
                        <TableCell>
