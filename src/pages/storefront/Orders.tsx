...
import type { Order, Page } from '@/services/types';
...
export default function Orders() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      // Try to fetch custom page for customer orders
      try {
        const ordersPage = await coreApi.getPageBySlug('customer-orders');
        if (ordersPage && ordersPage.isPublished && 
            ordersPage.content?.sections && 
            Array.isArray(ordersPage.content.sections) && 
            ordersPage.content.sections.length > 0) {
          setCustomPage(ordersPage);
        }
      } catch (e) {
      ...
      let regularOrdersResponse: { data: Order[] } | Order[] = { data: [] };
      ...
      const allOrders = [
        ...regularOrders,
        ...cardOrders.map((o: Order) => ({ ...o, isCardOrder: true })),
  ...
  if (isNative) return <MobileOrders />;
