...
export default function BuyerWallet() {
  ...
  const loadWalletData = async () => {
    ...
    try {
      ...
      // Generate mock invoices for previous months
      const mockInvoices: MonthlyInvoice[] = [
        {
          id: 'INV-2024-01',
          month: '2024-01',
          totalSpent: 850.00,
...
Month: ${invoice.month}
Date Generated: ${new Date(invoice.generatedAt).toLocaleDateString()}

-------------------------------------------------
SUMMARY
-------------------------------------------------

Total Spent:      ${invoice.totalSpent.toFixed(2)} SAR
Total Deposits:   ${invoice.totalDeposits.toFixed(2)} SAR
Transactions:     ${invoice.transactionCount}
...
=================================================
    ...
    a.href = url;
    a.download = `Invoice-${invoice.month}.txt`;
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="transactions" className="w-full">
        <TabsList className="w-full md:w-auto grid grid-cols-2 md:inline-flex">
          ...
          <TabsTrigger value="invoices">الفواتير الشهرية</TabsTrigger>
        </TabsList>
            ...
            <CardHeader className="border-b">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  <CardDescription>
                    يتم إصدار الفاتورة الشهرية في يوم 3 من كل شهر
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {invoices.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">لا توجد فواتير</p>
                </div>
              ) : (
                <div className="divide-y">
                    ...
                    <div key={invoice.id} className="p-4 hover:bg-muted/30 transition-colors">
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium text-sm">فاتورة شهر {invoice.month}</p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-xs text-muted-foreground flex items-center gap-1">
                                <Calendar className="h-3 w-3" />
                                تم الإصدار: {new Date(invoice.generatedAt).toLocaleDateString('ar-SA')}
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-6">
                          <div className="text-left hidden md:block">
                            ...
                            <p className="font-bold">{invoice.totalSpent.toFixed(2)} ر.س</p>
                          </div>
                          <Button 
                            variant="outline" 
                            size="sm" 
                  ...
                  <div className="p-4 bg-muted/20">
                    <div className="flex items-center gap-3 text-sm text-muted-foreground">
                      ...
                      <span>الفاتورة القادمة ستصدر في 3 {new Date(new Date().setMonth(new Date().getMonth() + 1)).toLocaleString('ar-SA', { month: 'long' })}</span>
                    </div>
                  </div>
                </div>
              )}
