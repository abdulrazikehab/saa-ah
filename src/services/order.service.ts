...
export const orderService = {
  createOrder: (data: CreateOrderData, requireAuth: boolean = true, options?: ApiOptions): Promise<Order> =>
  ...
  getOrders: async (params?: { page?: number; limit?: number; status?: string; search?: string; startDate?: string; endDate?: string }): Promise<Order[] | { data: Order[]; meta: { total: number; page: number; limit: number; totalPages: number } }> => {
  ...
  refundOrder: (id: string, reason?: string): Promise<Order> =>
    apiClient.fetch(`${apiClient.coreUrl}/orders/${id}/refund`, {
      method: 'PUT',
      body: JSON.stringify({ reason }),
      requireAuth: true,
    }),
  ...
  retryDelivery: (id: string): Promise<Order> =>
    apiClient.fetch(`${apiClient.coreUrl}/orders/${id}/retry-delivery`, {
      method: 'POST',
      requireAuth: true,
    }),
};
