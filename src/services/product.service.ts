...
export const productService = {
  ...
  createProduct: async (data: CreateProductData, upsert?: boolean, options: any = {}): Promise<Product> => {
  ...
  // Categories
  getCategories: async (params?: { page?: number; limit?: number; all?: boolean }, requireAuth = false): Promise<Category[] | { categories: Category[]; meta: any }> => {
  ...
  createCategory: async (data: CreateCategoryData, options: any = {}): Promise<{ message: string; category: Category }> => {
  ...
  // Brands
  getBrands: async (params?: Record<string, any>, requireAuth = false): Promise<any[] | { data: any[]; meta: any }> => {
    const queryString = params ? `?${new URLSearchParams(params as any)}` : '';
    const response = await apiClient.fetch(`${apiClient.coreUrl}/brands${queryString}`, {
      requireAuth,
    });
    // Handle paginated response { data: [...], meta: ... }
    if (response && typeof response === 'object' && 'data' in response && 'meta' in response) {
      return response as { data: any[]; meta: any };
    }
    // Handle response { data: [...] } without meta (some APIs do this)
  ...
  createBrand: async (data: Record<string, unknown>, options: any = {}): Promise<any> => {
